...
export default function DashboardPage() {
  ...
  const todayActions = useMemo(() => {
    if (bundle) {
      // Deduplicate by quote_id — the RPC can return both a 'followup'
      // and a 'viewed_hot' row for the same quote. Keep the higher-priority one.
      const seen = new Map();
      for (const a of (bundle.today_actions || [])) {
        if (dismissedIds.has(a.quote_id || a.invoice_id || a.booking_id)) continue;
        const key = a.quote_id || a.invoice_id || a.booking_id;
        if (!key) continue;
        const existing = seen.get(key);
        if (!existing || (a.priority ?? '9') < (existing.priority ?? '9')) {
          seen.set(key, a);
        }
      }
