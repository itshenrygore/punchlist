...
/* Show Foreman FAB on all authenticated app pages except quote builder/edit
   where Foreman is already embedded inline in the sidebar. */
function shouldShowForeman(pathname) {
  // Never show on the quote builder — it has an inline Foreman panel
  ...
  // Show on all other authenticated app routes
...
export default function Foreman() {
  ...
  const [triggerPos, setTriggerPos] = useState({ bottom: 88, right: 16 });
  ...
  function handleDragStart(e) {
    ...
    function move(ev) {
      ...
      setTriggerPos({
        ...
        bottom: Math.max(16, Math.min(window.innerHeight - 56, startBottom + dy)),
      });
    }
    function end() {
