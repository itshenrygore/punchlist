...
export default async function handler(req, res) {
  ...
  let authUser, authErr;
  try {
    const result = await supabaseCheck.auth.getUser(authToken);
  ...
  try {
    ...
    const stats = {
      ...
      approvedQuotes: allQuotes.filter(q => ['approved','approved_pending_deposit','deposit_paid','converted_to_invoice','paid'].includes(q.status)).length,
    };
