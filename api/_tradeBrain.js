...
export function summarizeHistory(historicalQuotes) {
  ...
  for (const row of quotes) {
    ...
    if (['approved', 'approved_pending_deposit', 'deposit_paid', 'converted_to_invoice', 'paid'].includes(row.status)) wonCount++;
    ...
    for (const item of items) {
      ...
      cur.count += qty;
      cur.total += price * qty;
      if (['approved', 'approved_pending_deposit', 'deposit_paid', 'converted_to_invoice', 'paid'].includes(row.status)) cur.wins++;
      itemMap.set(key, cur);
    }
  }
