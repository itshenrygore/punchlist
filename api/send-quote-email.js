...
export default async function handler(req, res) {
  ...
  if (body.action === 'payment_reminder') {
    ...
    const invoiceUrl = shareToken ? `${appUrl}/i/${shareToken}` : '';
  ...
  try {
    ...
    if (!isPro && quote.status !== 'sent') {
      ...
      const { count: sentCount, error: countErr } = await supabase
        ...
        .in('status', ['sent', 'viewed', 'revision_requested', 'approved', 'approved_pending_deposit', 'deposit_paid', 'converted_to_invoice', 'paid'])
