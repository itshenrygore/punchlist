...
function TimeGrid({ days: dayColumns, bookings, onSlotClick, onBlockClick, onDragReschedule }) {
  ...
  useEffect(() => {
    function updateNow() {
      ...
      setNowPos((min - DAY_START * 60) * (HOUR_HEIGHT / 60));
      else setNowPos(null);
    }
    updateNow();
  ...
  useEffect(() => {
    ...
    // Scroll to 7am so business hours are visible; early/late jobs reachable by scrolling
    const SCROLL_TO_HOUR = 7;
    scrollRef.current.scrollTop = (SCROLL_TO_HOUR - DAY_START) * HOUR_HEIGHT;
  }, []);
