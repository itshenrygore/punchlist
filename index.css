...
export default function QuoteBuilderPage() {
  ...
  useEffect(() => {
    ...
    profileP.then(p => {
      ...
      if (!isPro(p)) listQuotes(user.id).then(q => setSentThisMonth(countSentThisMonth(q || []))).catch(() => {});
    }).catch(() => {});
  ...
  async function save(nextStatus = null, silent = false) {
    ...
    const savePromise = (async () => {
      ...
      try {
        ...
        if (offlineDraft) { deleteOfflineDraft(quoteId).catch(() => {}); setOfflineDraft(false); }
        if (!silent) {
