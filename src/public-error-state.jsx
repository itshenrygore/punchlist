...
/**
 * RoutedShell — lives inside <BrowserRouter> so it can read the current
 * location. The pathname is passed as `resetKey` so that when the user
 * navigates (via the bottom nav, sidebar, browser back, etc.) the
 * ErrorBoundary clears its error automatically. Previously a single render
 * crash on any page would replace the entire app shell with the error
 * screen and leave the user with no nav — only a hard reload could escape.
 */
