...
const TRADE_PRESETS = {
  Plumber: [
    { label: 'Replace a leaking kitchen faucet', text: 'Replace leaking kitchen faucet. Disconnect old, install new Moen single-handle, replace shut-off valves and supply lines, test for leaks.' },
  ],
  Electrician: [
...
function bumpDemo() { const d = new Date().toISOString().slice(0, 10), c = demoCount() + 1; try { localStorage.setItem(DK, JSON.stringify({ count: c, date: d })); } catch {} }
      ...
      {phase === 'results' && (
        <div className="id-results">
          <div className="id-scope-bar">
            ...
            {aiSource === 'ai' && <span className="id-ai-badge">AI scope</span>}
          </div>
          ...
          {doneRevealing && total > 0 && (
            <button className="id-preview-toggle" type="button" onClick={() => setShowPreview(p => !p)}>
              {showPreview ? '▾ Hide customer view' : '▸ See what your customer sees'}
            </button>
          )}
