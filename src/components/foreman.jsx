...
export default function Foreman() {
  ...
  // Allow external trigger (sidebar button)
  useEffect(() => {
    const orig = window.__punchlistOpenForeman;
    window.__punchlistOpenForeman = () => setOpen(true);
  ...
  const [triggerPos, setTriggerPos] = useState({ bottom: 76, right: 16 });
  ...
  function handleDragStart(e) {
    ...
    function move(ev) {
      ...
      setTriggerPos({
        ...
        bottom: Math.max(66, Math.min(window.innerHeight - 56, startBottom + dy)),
      });
    }
    function end() {
  ...
  return (
    <>
      ...
      {!open && (
        ...
        >
          <span className="fm-trigger-icon">💬</span>
        </button>
      )}
        ...
        <div className="fm-overlay" onClick={() => setOpen(false)}>
          <div className="fm-panel" onClick={e => e.stopPropagation()} role="dialog" aria-label="Foreman assistant" aria-modal="true">
            ...
            <div className="fm-input-bar">
              <button className="fm-input-btn" type="button" onClick={() => fileRef.current?.click()} aria-label="Attach photo">📷</button>
              <input hidden ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handlePhoto} />
              <button className="fm-input-btn" type="button" onClick={handleVoice} aria-label="Voice input">🎤</button>
              <input
                ref={inputRef}
                className="fm-input"
                value={input}
