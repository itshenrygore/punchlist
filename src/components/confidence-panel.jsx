...
export default function ConfidencePanel({ confidence }) {
  ...
  const readinessColor = readiness==='ready' ? 'var(--green)' : readiness==='review' ? 'var(--amber)' : 'var(--red)';
  return (
    <div className="conf-panel">
      <div className="conf-panel-top">
        <div>
          <div className="eyebrow" style={{ marginBottom:2 }}>Quote confidence</div>
          <div style={{ fontSize:11, color:readinessColor, fontWeight:700 }}>{readinessLabel}</div>
        </div>
