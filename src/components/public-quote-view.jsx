...
export default function PublicQuoteView({
  ...
  const isExpired = useMemo(() => quote?.expires_at && new Date(quote.expires_at) < new Date() &&
    !['approved','approved_pending_deposit','deposit_paid','converted_to_invoice','paid'].includes(quote?.status), [quote]);
  ...
  const canUndoDecline = useMemo(() => {
    if (!quote?.declined_at) return true;
    const hoursSince = (Date.now() - new Date(quote.declined_at).getTime()) / 3600000;
  ...
  async function handleUndoDecline() {
    ...
    try {
      const r = await fetch('/api/public-quote-action', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: shareToken, action: 'undo_decline' }),
      });
      const data = await r.json();
      if (!r.ok) throw new Error(data.error || 'Failed');
      _setQuote(prev => ({ ...prev, status: data.status || 'viewed', declined_at: null }));
      setActionDone('');
    } catch (e) { setError(e.message || 'Could not undo decline'); }
  ...
  const { canAct: _canAct, canSign: _canSign, isClosedOut } = isPreview
    ? { canAct: false, canSign: false, isClosedOut: false }
  ...
  const isConvertedOrPaid = ['deposit_paid','converted_to_invoice','paid'].includes(quote.status);
        ...
        <div className="doc-card">
          ...
          {actionDone === 'decline' && !isPreview && (
            <div style={{ padding: '0 28px 8px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <button className="doc-cta-primary" type="button" style={{ width: '100%', background: 'var(--doc-accent)' }} onClick={handleUndoDecline} disabled={undoing}>
                {undoing ? 'Reopening…' : 'Changed my mind — I want to proceed'}
              </button>
              <button className="pq-btn-secondary" type="button" style={{ width: '100%' }} onClick={() => setActiveSheet('question')}>
                <MessageSquare size={14} style={{verticalAlign:'middle',marginRight:6}}/>Message {contractorDisplayName}
              </button>
            </div>
          )}
          ...
          <div className="doc-pricing-hero">
            ...
            <div className="doc-meta-row">
              ...
              {quote.schedule_window && <span className="doc-meta-chip">{quote.schedule_window}</span>}
            </div>

            {/* Customer + address context */}
            {quote.customer_address && (
              <div className="doc-address">{quote.customer_address}</div>
            )}
          </div>
          ...
          {optionalItems.length > 0 && (
            <div className="doc-items" style={{ paddingTop: 0 }}>
              <div className="doc-group doc-group--optional">
                ...
                {/* Always show the explanation. The previous version
                    hid it once the quote was approved or closed,
                    leaving a row of switches with no context for
                    anyone scrolling back through the document. */}
                <div className="pq-optionals-intro">
                  {canAct
                    ...
                    : 'These add-ons were available alongside the main scope.'}
                </div>
          ...
          <div className="pq-actions">
            ...
            {!canAct && !isPreview && !isConvertedOrPaid && !isClosedOut && (isSigned || isApproved) && (
          ...
          {/* Viral loop — contractor-facing CTA, tasteful attribution */}
          {mode !== 'portal-tab' && (
          <a
            href="https://punchlist.ca?ref=quote"
            target="_blank"
            rel="noreferrer"
            className="doc-powered-by"
          >
