...
function TimeGrid({ days: dayColumns, bookings, onSlotClick, onBlockClick, onDragReschedule }) {
 ...
 {HOURS.map(h => <div key={h} className="sched-hour-label">{fmtHour(h)}</div>)}
 </div>
 ...
 {isToday && nowPos !== null && <div className="sched-now-line"><div className="sched-now-dot" /></div>}
 sched-ghost cal-none-4e54{showEmptyHint && <div className="sched-empty-hint">Tap to schedule</div>}
 <div className="sched-ghost" />
 {dayBookings.map(b => {
