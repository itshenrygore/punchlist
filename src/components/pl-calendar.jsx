...
/* compact calendar duration: 90 → "1h30m" */
const fmtDuration = (min) => _fmtDuration(min, true);

/* ═══════════════════════════════════════════════════════════
   PlCalendar — v3: Premium Schedule Workspace
   Views: week (default desktop) | day (default mobile) | month
   Improvements: cleaner blocks, better mobile, smoother drag,
   improved ghost slot preview, tighter spacing
   ═══════════════════════════════════════════════════════════ */
...
function fmtMinutes(totalMin) {
  ...
  const ap = h24 >= 12 ? 'p' : 'a', h = h24 % 12 || 12;
...
function statusClass(status) {
  if (status === 'confirmed') return 'sched-block-confirmed';
  if (status === 'completed') return 'sched-block-completed';
  return 'sched-block-scheduled';
}
...
function MonthView({ bookings, viewDate, onSelectDate, selectedDate, onNav, onBlockClick, onSlotClick }) {
  ...
  return (
    <div className="pl-cal" {...monthSwipe}>
      ...
      <div className="pl-cal-dow">{DOW_TINY.map(d => <div key={d} className="pl-cal-dow-label">{d}</div>)}</div>
        ...
        {days.map((cell, i) => {
          ...
          return (
            <div key={i}
              ...
              aria-label={cell.other ? undefined : `${MONTHS[cell.month]} ${cell.day}, ${bHere.length} bookings`}
            >
      ...
      {selectedDate && (() => {
        ...
        return (
          <div className="pl-cal-day-cards">
            ...
            {active.length === 0 && (
              <>
                <div className="pl-cal-day-empty">No bookings scheduled</div>
                {onSlotClick && (
            ...
            {active.map(b => {
              ...
              return (
                <div key={b.id} className={`pl-cal-card ${statusClass(b.status)}`}
...
function TimeGrid({ days: dayColumns, bookings, onSlotClick, onBlockClick, onDragReschedule }) {
  ...
  const gutterW = isMobileGrid ? 36 : 48;
  ...
  const bookingMap = useMemo(() => {
    ...
    bookings.forEach(b => {
      ...
      // For multi-day bookings (>8h), show on each day they span
      if (dur > 480) {
        const days = Math.ceil(dur / 480);
        for (let i = 0; i < days; i++) {
          const spanDay = new Date(d);
          spanDay.setDate(spanDay.getDate() + i);
          const key = dateKey(spanDay);
          (m[key] ||= []).push({ ...b, _isSpanDay: i > 0 });
        }
      } else {
        (m[dateKey(d)] ||= []).push(b);
      }
        ...
        {dayColumns.map((day, i) => {
          ...
          return (
            <div key={i} className={`sched-day-header ${isToday ? 'sched-today-header' : ''}`}>
              <span className="sched-dow">{DOW_SHORT[day.getDay()]}</span>
              ...
              return (
                <div key={colIdx} className={`sched-col ${isToday ? 'sched-col-today' : ''}`}
                  ...
                  {dayBookings.map(b => {
                    ...
                    const depositPaid = b.quote?.deposit_required && b.quote?.deposit_status === 'paid';
                    const depositPending = b.quote?.deposit_required && b.quote?.deposit_status !== 'paid';
                    const name = b.customer?.name || 'Direct booking';
                    const timeLabel = formatTimeShort(b.scheduled_for);
                    return (
                      <div key={b.id}
                        className={`sched-block ${statusClass(b.status)} ${isDraggable(b.status) && !IS_TOUCH ? 'sched-block-draggable' : ''}`}
                        style={{ top, height: blockHeight, left: `calc(${laneLeft}% + 2px)`, width: `calc(${laneWidth}% - 4px)` }}
                      ...
                      >
                        {/* Row 1: Name + signal badges */}
                        <div className="sched-block-row1">
                          <span className="sched-block-name">{name}</span>
                          {b.assigned_to && <span className="sched-block-signal sched-signal-crew" title={`Assigned to ${b.assigned_to}`}>{b.assigned_to.slice(0, 2)}</span>}
                          {depositPending && <span className="sched-block-signal sched-signal-deposit" title="Deposit pending">$</span>}
                          {depositPaid && <span className="sched-block-signal sched-signal-paid" title="Deposit paid">✓</span>}
                        </div>
                        {/* Row 2: Job title (if room) */}
                        {!isTiny && b.quote?.title && <span className="sched-block-job">{b.quote.title}</span>}
                        {/* Row 3: Time + duration (if room) */}
                        {!isCompact && (
                          <span className="sched-block-time">
                            {timeLabel} · {fmtDuration(b.duration_minutes || 120)}
                            {(b.duration_minutes || 0) > 480 && ' (multi-day)'}
                          </span>
                        )}
                        {/* Map pin for day view only */}
...
export default function PlCalendar({ bookings = [], view, viewDate, onViewChange, onViewDateChange, onSlotClick, onBlockClick, onDragReschedule, selectedDate, onSelectDate }) {
  ...
  function navWeek(dir) {
    ...
    // On mobile 3-day view, navigate by 1 day. On desktop, navigate by 7 days.
  ...
  if (view === 'week') {
    // On mobile: show 3-day window centered on viewDate. On desktop: full 7-day week.
    let displayDays;
    let monthLabel;
    if (isMobile) {
      ...
      <div className="sched-container" {...swipeHandlers}>
        <div className="sched-toolbar">
          ...
          <div className="sched-toolbar-right">
            <button className="pl-cal-nav-btn" type="button" onClick={() => navWeek(-1)} aria-label={isMobile ? "Previous day" : "Previous week"}>‹</button>
            <button className="pl-cal-nav-btn sched-today-btn" type="button" onClick={() => navWeek(0)} aria-label="Today">Today</button>
            <button className="pl-cal-nav-btn" type="button" onClick={() => navWeek(1)} aria-label={isMobile ? "Next day" : "Next week"}>›</button>
          </div>
        </div>
      ...
      <div className="sched-container" {...swipeHandlers}>
        <div className="sched-toolbar">
          ...
          <div className="sched-toolbar-right">
            ...
            <button className="pl-cal-nav-btn sched-today-btn" type="button" onClick={() => navDay(0)} aria-label="Today">Today</button>
