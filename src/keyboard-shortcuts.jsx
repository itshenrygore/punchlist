...
export default function Foreman() {
  ...
  const [triggerPos, setTriggerPos] = useState(() => {
    try {
      const raw = localStorage.getItem('pl_foreman_pos');
      if (raw) {
        const p = JSON.parse(raw);
        if (typeof p?.bottom === 'number' && typeof p?.right === 'number') return p;
      }
    } catch { /* no-op */ }
    return { bottom: 136, right: 16 };
  });
  ...
  useEffect(() => {
    try { localStorage.setItem('pl_foreman_pos', JSON.stringify(triggerPos)); } catch { /* no-op */ }
  }, [triggerPos]);

  // Keep within viewport on resize / orientation change
  useEffect(() => {
    function clamp() {
      setTriggerPos(p => ({
        right: Math.max(8, Math.min(window.innerWidth - 64, p.right)),
        bottom: Math.max(96, Math.min(window.innerHeight - 64, p.bottom)),
      }));
    }
  ...
  // Drag handler — attached as a NATIVE non-passive listener via useEffect.
  // React's synthetic onTouchStart is passive by default, so e.preventDefault()
  // is silently ignored and the page scrolls instead of the FAB moving.
  // Using Pointer Events covers mouse + touch + pen with one path.
  useEffect(() => {
    ...
    function onPointerDown(e) {
      ...
      startX = e.clientX;
      startY = e.clientY;
      startRight = triggerPos.right;
      startBottom = triggerPos.bottom;
    ...
    function onPointerMove(e) {
      ...
      if (dragMoved.current) {
        ...
        setTriggerPos({
          ...
          bottom: Math.max(96, Math.min(window.innerHeight - 64, startBottom + dy)),
        });
      }
    }
    ...
    function onPointerUp(e) {
      ...
      // Snap to nearest horizontal edge so the FAB always feels docked.
      if (dragMoved.current) {
        setTriggerPos(p => {
          const centerX = window.innerWidth - p.right - 28; // trigger ≈56px
          const snapRight = centerX < window.innerWidth / 2
            ? window.innerWidth - 64 // dock left  (right = innerWidth - 64)
            : 16;                    // dock right (default)
          return { ...p, right: snapRight };
        });
      }
    }

    // Non-passive so preventDefault() actually works on iOS Safari
  ...
  }, [triggerPos.right, triggerPos.bottom]);
    ...
    <>
      ...
      {!open && showTrigger && (
        <button
          ...
          style={{ bottom: triggerPos.bottom, right: triggerPos.right, touchAction: 'none' }}
          onClick={handleTriggerClick}
          ref={dragRef}
          aria-label="Open Foreman"
        >
          <span className="fm-trigger-icon"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></span>
        </button>
      )}
