...
function PushToggle({ userId }) {
 ...
 if (smuted small sp-s42-9ac5unsupported') return <div className="muted small">Push notifications are not supported in this browser.</div>;
 if (status === 'denied') return <div className="muted small" >Notifications are blocked. Enable them in your browser settings to receive push alerts.</div>;

 if (status === 'subscribed') {
 return (
 <div>
 <div className="sp-flex_fs-sm-4db3"Weight: 600 }}>✓ Push notifications enabled</div>
 <button className="btn btn-secondary btn-sm" type="button" disabled={working} onClick={unsubscribe} >
...
export default function SettingsPage() {
 ...
 try { return sessionStorage.getItem('pl_settings_tab') || 'business'; }
 ...
 <div>
 <span className="field-label">Logo (shown on qbtn btn-secondary btn-sm sp-s41-911cn>
 <div className="sp-flex-a37f">
 ...
 input sp-s10-a09b</label>
 <span className="sp-fs-2xs-cf07">or paste a URL:</span>
 </div>
 <input className="input" value={form.logo_url || ''} onChange={e => setForm(p => ({ ...p, logo_url: e.target.value }))} placeholder="https://yoursite.com/logo.png" />
 {form.logo_url && (
 <div className="sp-flex-c25f">
 <img src={form.logo_url} alt="Logo preview" className="sp-s40-b1ce"69e0--line)' }} onError={e => { e.target.style.display = 'none'; }} />
 <button type="button" className="btn btn-secondary btn-sm" onClick={() => setForm(p => ({ ...p, logo_url: '' }))}>Remove</button>
 </div>
 )}
 </div>
 <div className="form-row">
 ...
 <div className="sp-flex-4579">
 <span className="sp-s39-aeee">$</span>
 <input className="input" type="number" min="0" step="5" value={form.default_labour_rate || ''} onChange={e => setForm(p => ({ ...p, default_labour_rate: Number(e.target.value) || 0 }))} placeholder="e.g. 165" className="sp-s38-39e8" />
 <span className="sp-fs-xs-b2de">/hr</span>
 </div>
 <div className="sp-fs-2xs-171d">Used for labour items on new quotes. Leave blank for suggested rates.</div>
 </div>
 </div>
 <div>
 ...
 <dinput sp-s37-dab4 <span className="field-label">Account email (where approval notifications are sent)</span>muted small sp-s26-12e2 <input className="input" value={user?.email || ''} readOnly />
 <div className="muted small" >This is your login email. Change it through your account provider.</div>
 </div>
 </div>
 </div>
 ...
 >
 {EXPmuted small sp-s36-3043.map(o => (
 ...
 muted small sp-s35-6b09 {/* ═══ QUOTE TRACKING — explainer ═══ */}
 <div className="panel">
 ...
 <div className="sp-grid_fs-xs-2d9c">
 ...
 {/* ═══ QUOTING & PAYMENTS TAB (was Payments + Messages) ═══ */}
 {settingsTab === 'quoting' && <>
 ...
 <div className="sp-s34-8dc6">
 <div className="sp-flex-1c80">
 <span className="sp-fs-base-f4e6">Punchlist Payments</span>
 {connectLoading && !connectStatus && (
 <span className="sp-inline-block-5f05" />
 )}
 {connectStatus?.onboarded && <span className="sp-fs-2xs-95ae">Active</span>}
 </div>
 {connectStatus === null && !connectLoading ? (
 <div className="sp-flex-0c06">
 <div className="sp-s33-40b6" />
 <div className="sp-s32-96d0" />
 <div className="sp-s31-e135" />
 </div>
 ) : !connectStatus?.onboarded ? (
 <>
 <div className="sp-fs-xl-081e"Bottom: 6 }}>
 ...
 <div className="sp-grid_ta-center-5e40">
 ...
 btn btn-primary sp-ta-center_fs-base-8447iv className="muted-small">Paid upfront</div>
 </div>
 </div>
 ...
 <div className="sp-ta-center_fs-2xs-2243">
 Nmuted small sp-s29-b27cee · No setup fee · Small processing fee per transaction
 </div>
 </>
 ) : (
 <>
 ...
 <div className="sp-flex-3f49"rap: 'wrap' }}>
 <button type="button" className="btn btn-secondary" onClick={openStripeDashboard} disabled={stripeDashLoading} >
 {stripeDabtn btn-secondary sp-fs-xs-f1daing…' : 'View Stripe Dashboard →'}
 </button>
 <Link to="/app/payments-setup" className="btn btn-secondary" >
 How it works →
 </Link>
 </div>
 </>
 ...
 {/* ── Deposit defaults ── */}
 <div className="sp-s28-eb4a">
 <div className="sp-fs-xs-1003">Deposits</div>
 <div className="stack">
 <div>
 ...
 {/* ── Invoice terms ── */}
 <div className="sp-s28-eb4a">
 <div className="sp-fs-xs-1003">Invoice terms</div>
 <div className="stack">
 <div>
 ...
 muted small sp-s27-026e <div className="sp-fs-xs-459e">Other payment methods</div>
 <p className="muted small" >
 ...
 <input className="input" value={form.stripe_pamuted small sp-s26-12e2 onChange={e => setForm(p => ({ ...p, stripe_payment_link: e.target.value }))} placeholder="https://buy.stripe.com/XXXXXXX" />
 <div className="muted small" >Paste your own Stripe payment link as a fallback.</div>
 </div>
 )}
 </div>
 </div>
 ...
 <div>
 input textarea-md sp-fs-sm-3705lassName="field-label">Terms &amp; conditions text (optional)</span>
 <textarea
 className="input textarea-md"
 
 value={form.terms_conditions}
 onChange={e => setForm(p => ({ ...p, terms_conditions: e.target.value }))}
 placeholder="e.g. All work is guaranteed for 1 year from the date of completion. A 25% cancellation fee applimuted small sp-s10-a09bis cancelled within 48 hours of the scheduled date. Customer is responsible for providing clear access to the work area…"
 />
 <p className="muted small" >Leave blank to hide this section from stack sp-s24-9313s.</p>
 </div>
 </div>
 ...
 <button className="btn btn-secondary" type="button" disabled={pwSaving} onClick={panel sp-s25-80dcePw}>
 ...
 {/* Subscription */}
 <div className="panel" 51 var(--brand-line)' }}>
 <div className="eyebrow">Subscription</div>
 <div className="stack" >
 <p className="muted small" >
 Upgrasettings-pricing-grid sp-grid-d3eded quotes, Foreman scope checking, deposits, scheduling, invoicing, and customer pay-over-time.
 </p>
 <div className="settings-pricing-grid" >
 <div className="sp-ta-center-c2f9">
 <div className="sp-s22-dcb5"p-fs-2xs-5c5e}}>${PRICING.monthly}</div>
 <div className="muted small">per month</div>
 <button className="btn btn-secondary full-width" type="button" onClick={() => navigate('/app/billing')}>Monthly plan</button>
 </div>
 <div className="sp-ta-center-3629">
 <div className="sp-fs-2xs-d1f6">BEST VALUE</div>
 <div className="sp-s22-dcb5"n btn-primary full-width sp-fs-2xs-5c5e
 <div className="muted small">per year · save ${PRICING.annualSavings}</div>
 <button className="btn btn-primary full-width" ae(--text-2xs)' }} type="button" onClick={() => navigate('/app/billing')}>Yearly plan</button>
 </div>
 </div>
 <div className="muted small" >30-day free trial · no credit card required · cancel anytime</div>
 </div>
 </div>

 {/* ═══ MESSAGES (merged into Quoting & Payments) ═══ */}
 {/* Intro / orientation */}
 <div className="panel">
 ...
 {!proAccess && (
 <div
 className="sp-fs-xs-2bdc"
 >
 You're on the free plan — you can <strong>preview</strong> every
 message and <strong>reset</strong> any customizapanel sp-s21-7e60
 ...
 <div className="panel" >
 <div className="sp-flex-7d32"ted small sp-s19-5302 }}>
 <div className="sp-s17-30f9">
 <div className="eyebrow" >Nudge schedule</div>
 <p className="muted small" >
 ...
 </divmuted small sp-s18-0128 <div className="sp-flex-523f">
 <div className="muted small" >
 ...
 <div
 className="sp-flex-5f0a"
 >
 <div className="sp-s17-30f9">
 <div className="sp-fs-sm-1549"Bottom: 2 }}>
 ...
 <a className="btn btn-primary btn-sm" href="/settings?tab=bpanel sp-s3-4c92g">
 See Pro
 </a>
 </div>
eyebrow sp-s15-8766 </div>
 )}
 </div>

 {/* Token legend */}
 <div className="panel" >
 <div className="eyebrow" >Tokens you can use</div>
 <div
 className="sp-flex_fs-2xs-8f2d"
 >
 {[
 ...
 {/* ═══ ACCOUNT TAB (was Account + Notifications + Preferences) ═══ */}
 {settingsTab === 'account' && <>
 {/* ── Notifications ── */}
 {/* 7E: Daily Digest */}
 <div className="panel">
 ...
 <input type="checkbox" checked={form.digest_enabled} onChange={e =muted small sp-s12-e058 => ({ ...p, digest_enabled: e.target.checked }))} className="settings-checkbox" />
 ...
 <div className="sp-fs-xs-c38f">
 Add your phone number above to receive SMS notifications.
 </div>
 )}
 {form.sms_notifications_enabled && form.phone?.trim() && (
 <div className="sp-fs-xs-bc5e"ll sp-s11-188f ✓ Texts will be sent to <strong>{form.phone}</strong> when customers act on your quotes.
 </div>
 )}
 <div className="muted small" >
 <strong>You'll get texts when:</strong> customer views your quote, approves, requests changes, declines, or asks a question.
 <br />
 <strong>Your customers get texts when:</strong> you smuted small sp-s10-a09b, schedule a job, reschedule, send an invoice, or reply to their question — only if they have a phone number on file.
 </div>
 <div className="muted small" >
 ...
 <div className="sp-flex-82e1">
 muted small sp-s8-1d91ssName="sp-s9-f72f">
 <div className="sp-fs-base-67f2">Auto-send invoice on complete</div>
 <div className="muted small" >
 ...
 <div key={s.keys + s.label} className="sp-contents-0573">
 ...
 <hrbtn btn-secondary sp-s6-622c-4d5d" />
 {!deleteConfirm ? (
 <button className="btn btn-secondary" type="button" onClick={() => setDeleteConfirm(true)}>Delete my account</button>
 ) : !deleteConfirm2 ? (
 <div>
 <p className="sp-fs-sm-d98d">Are you sure? This will permanently delete all your quotes, contacts, invoices, and bookings.</p>
 <div className="sp-flex-84cf">
 btn btn-secondary sp-s5-4891Name="btn btn-secondary" type="button" onClick={() => setDeleteConfirm(false)}>Cancel</button>
 <button className="btn btn-secondary" type="button" onClick={() => setDeleteConfirm2(true)}>Yes, I understand</button>
 </div>
 </div>
 ) : (
 <div>
 <p className="sp-fs-sm-1ab1">Final confirmation — this cannot be undone.</p>
 <div className="sp-flex-84cf">
 <button className="btn btn-secondary" type="bbtn btn-primary sp-s4-c70f{() => { setDeleteConfirm(false); setDeleteConfirm2(false); }}>Cancel</button>
 <button className="btn btn-primary" type="button" disabled={deleting} onClick={handleDeleteAccount}>
 {deleting ? 'Deleting…' : 'Permanently delete epanel sp-s3-4c92hing'}
 </button>
 </div>
 </div>
 )}
...
stack sp-s2-aa90 {/* About */}
 <div className="panel" anel-2)' }}>
 ...
 <div className="muted small">
 Questions? Email <a href="mailto:hello@punchlist.ca" className="sp-s0-feea">hello@punchlist.ca</a>
 </div>
 </div>
 </div>
 </>}
