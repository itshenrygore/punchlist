  ...
  useEffect(() => {
    ...
    Promise.all([getProfile(user.id), listQuotes(user.id)]).then(([p, q]) => {
      ...
      setStats({
        ...
        won: quotes.filter(x => ['approved','approved_pending_deposit','deposit_paid','converted_to_invoice','paid'].includes(x.status)).length,
      });
    ...
    <AppShell title="Billing">
      <div style={{ maxWidth: 600, margin: '0 auto', display: 'grid', gap: 16 }}>
        ...
        <div className="panel" style={{ textAlign: 'center', padding: 28 }}>
          ...
          <div style={{ fontSize: 'var(--text-4xl)', fontWeight: 800, letterSpacing: '-.03em', color: 'var(--text)', textTransform: 'capitalize' }}>
            {pro ? 'Pro' : 'Free'}
          </div>
          {pro && (
            <div style={{ fontSize: 'var(--text-sm)', color: 'var(--green)', fontWeight: 600, marginTop: 6 }}>
        ...
        {!pro && (
          <div className="panel" style={{ background: 'linear-gradient(160deg, var(--brand-bg), var(--panel))', border: '1px solid var(--brand-line)' }}>
            ...
            <p className="muted" style={{ fontSize: 'var(--text-base)', lineHeight: 1.6, marginTop: 8, marginBottom: 16 }}>
            ...
            <div className="billing-pricing-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div className="panel" style={{ textAlign: 'center', padding: 16 }}>
                <div style={{ fontSize: 'var(--text-3xl)', fontWeight: 800, letterSpacing: '-.03em' }}>${PRICING.monthly}</div>
                <div className="muted" style={{ fontSize: 'var(--text-xs)' }}>per month</div>
              ...
              <div className="panel" style={{ textAlign: 'center', padding: 16, border: '2px solid var(--green)', position: 'relative', overflow: 'visible', paddingTop: 20 }}>
                <div style={{ position: 'absolute', top: -9, left: '50%', transform: 'translateX(-50%)', background: 'var(--green)', color: 'white', fontSize: 'var(--text-2xs)', fontWeight: 800, padding: '2px 8px', borderRadius: 'var(--r-pill)', whiteSpace: 'nowrap', zIndex: 1 }}>SAVE ${PRICING.annualSavings}</div>
                <div style={{ fontSize: 'var(--text-3xl)', fontWeight: 800, letterSpacing: '-.03em' }}>${PRICING.annual}</div>
                <div className="muted" style={{ fontSize: 'var(--text-xs)' }}>per year · ${PRICING.annualMonthly}/mo</div>
            ...
            <div className="muted" style={{ fontSize: 'var(--text-xs)', textAlign: 'center', marginTop: 10 }}>Cancel anytime · upgrade or downgrade freely</div>
          </div>
        )}
        ...
        {pro && (
          <div className="panel">
            ...
            <p className="muted" style={{ fontSize: 'var(--text-sm)', marginTop: 8, marginBottom: 12 }}>
        ...
        <div className="panel" style={{ background: 'var(--panel-2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginTop: 10 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'var(--text-2xl)', fontWeight: 800, color: 'var(--text)' }}>{stats.quotes}</div>
              <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>Quotes</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'var(--text-2xl)', fontWeight: 800, color: 'var(--text)' }}>{stats.sent}</div>
              <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>Sent</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 'var(--text-2xl)', fontWeight: 800, color: 'var(--text)' }}>{stats.won}</div>
              <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>Won</div>
            </div>
          </div>
        </div>
        ...
        <div className="panel" style={{ background: 'var(--panel-2)' }}>
          ...
          <div style={{ marginTop: 8, fontSize: 'var(--text-base)', color: 'var(--text-2)' }}>{user?.email || ''}</div>
