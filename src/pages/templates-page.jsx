...
import {
  listJobTemplates, saveJobTemplate, deleteJobTemplate,
...
function JobTemplateModal({ tmpl, onSave, onClose, saving }) {
    ...
    <div className="jt-modal-bg" onClick={onClose}>
      <div className="jt-modal" onClick={e => e.stopPropagation()}>
        <div className="jt-modal-hd">
          <h3 className="jt-modal-title">{tmpl?.id ? 'Rename template' : 'Save as job template'}</h3>
...
export default function TemplatesPage() {
  ...
  const [editingTmpl, setEditingTmpl]     = useState(null); // tmpl being renamed/null
    ...
    <AppShell>
      <div className="tmpl-root">
        <div className="tmpl-page-hd">
          ...
          {activeTab === 'jobs' && jobTemplates.length > 0 && (
            <button
              type="button"
            ...
            {jobLoading ? (
              ...
              <div className="jt-empty">
                ...
                <p className="jt-empty-body">
                  Save any quote as a template from the quote builder or detail page.
                  Next time you do the same job, start from the template — trade, description,
                  province, and all line items pre-filled.
                </p>
                <Link to="/app/quotes/new" className="btn btn-primary">Build your first quote →</Link>
              </div>
            ) : (
