...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Activation email sequence — fire-and-forget, non-blocking
    fetch('/api/activation-email', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: user.id }) }).catch(() => {});
    Promise.all([listQuotes(user.id), listBookings(user.id), listInvoices(user.id), listCustomers(user.id)])
      .then(([q, b, inv, customers]) => {
        ...
        setInvoiceList(inv || []);
        // Onboarding trigger moved to getProfile callback so profileLoaded is guaranteed
    ...
    getProfile(user.id).then(p => {
      ...
      // Gate onboarding on profile being loaded to prevent false positive on returning users
    }).catch(() => {});
  }, [user]);
              ...
              {attentionItems.length > 0 && (() => {
                ...
                <div className="v2-feed" ref={attentionRef}>
                  <div className="v2-feed-head">
                    ...
                    <span className="v2-feed-count">{visibleItems.length} · {currency(visibleItems.reduce((s, i) => s + Number((i.quote?.total) || 0), 0))} on the line</span>
                  </div>
