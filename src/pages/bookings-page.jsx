...
export default function BookingsPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('invoices').select('id,quote_id').eq('user_id', user.id)
      .then(({ data }) => {
        if (data) setInvoiceQuoteIds(new Set(data.filter(i => i.quote_id).map(i => i.quote_id)));
      });
  }, [user]);
                  ...
                  <div className="sched-drawer-body">
                    ...
                    {drawerBooking.notes && (
                      <div className="sched-drawer-row">
                        ...
                        <span style={{ fontSize: 12, color: 'var(--muted)' }}>{drawerBooking.notes}</span>
                      </div>
                    )}
                  ...
                  <div className="sched-drawer-body">
                    ...
                    {editConflictWarning && editConflictWarning.length > 0 && (
                      <div className="sched-conflict-warn">
                          ...
                          <div className="sched-suggest-row">
                            ...
                            <div className="sched-suggest-chips">
                              {editSuggestedSlots.map((s, i) => (
                                <button key={i} type="button" className="sched-suggest-chip"
                        ...
                        <div style={{ marginTop: 6, fontSize: 11, color: 'var(--muted)' }}>Tap "Save" again to keep this time.</div>
                      </div>
                    )}
