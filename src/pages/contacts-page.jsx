...
function ActivityTimeline({ quotes, bookings, invoices }) {
  const events = useMemo(() => {
    const all = [];
    for (const q of quotes) all.push({ date: q.created_at, icon: 'quote', label: q.title || 'Untitled quote', sub: chipForStatus(q.status), path: `/app/quotes/${q.id}`, tone: toneForStatus(q.status) });
    for (const b of bookings) all.push({ date: b.scheduled_for, icon: 'booking', label: b.quote?.title || 'Booking', sub: b.status, path: null, tone: 'sent' });
    for (const i of invoices) all.push({ date: i.created_at, icon: 'invoice', label: i.invoice_number || 'Invoice', sub: i.status, path: `/app/invoices/${i.id}`, tone: i.status === 'paid' ? 'paid' : 'sent' });
        ...
        <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '10px 0', borderBottom: i < events.length - 1 ? '1px solid var(--line)' : 'none' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {ev.path
              ...
              : <span style={{ fontWeight: 600, fontSize: 'var(--text-sm)' }}>{ev.label}</span>}
            <div style={{ display: 'flex', gap: 8, marginTop: 3, alignItems: 'center' }}>
              <span className={`status-chip ${ev.tone}`} style={{ fontSize: 'var(--text-2xs)', padding: '1px 6px' }}>{ev.sub}</span>
              <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--subtle)' }}>{formatDate(ev.date)}</span>
            </div>
          </div>
        </div>
      ))}
              ...
              {filteredCustomers.length ? filteredCustomers.map(c => {
                ...
                return (
                  <button type="button" key={c.id} className={`ct-item ${selectedId === c.id ? 'active' : ''} ${c.archived_at ? 'archived' : ''}`} onClick={() => selectCustomer(c)}>
                    <div className="ct-avatar" style={{ background: ['var(--brand)','var(--blue, #2563eb)','var(--green)','var(--purple, #7c3aed)','var(--red)','var(--blue, #0891b2)'][(c.name||'').charCodeAt(0) % 6] }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {ctags.length > 0 && (
                        <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', marginTop: 3 }}>
                          ...
                          {ctags.length > 3 && <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--subtle)' }}>+{ctags.length - 3}</span>}
                        </div>
                      )}
                    </div>
          ...
          <section className="ct-detail">
              ...
              <div className="qb-card">
                ...
                <form onSubmit={saveCustomer} className="ct-form">
                  ...
                  <div className="ct-form-actions">
                    ...
                    {selected?.phone && <a className="btn btn-secondary btn-sm" href={`sms:${selected.phone}`} style={{ textDecoration: 'none' }}>Text</a>}
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                      {(Array.isArray(selected.tags) ? selected.tags : []).map(tag => (
                        <span key={tag} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 'var(--text-xs)', padding: '3px 8px', borderRadius: 99, background: tagColor(tag) + '22', color: tagColor(tag), border: `1px solid ${tagColor(tag)}55`, fontWeight: 700 }}>
                          {tag}
                          <button type="button" aria-label={`Remove tag ${tag}`} onClick={() => removeTag(tag)} className="jd-photo-dismiss" style={{ color: tagColor(tag) }}><X size={10} /></button>
                        </span>
                      ))}
                      {(Array.isArray(selected.tags) ? selected.tags : []).length === 0 && (
                        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--subtle)' }}>No tags yet</span>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <input
                  ...
                  {customerQuotes.length ? customerQuotes.map(q => (
                    <Link key={q.id} to={`/app/quotes/${q.id}`} className="ct-history-item">
                      <div>
                        ...
                        <span className={`status-chip ${toneForStatus(q.status)}`} style={{ fontSize: 'var(--text-2xs)', padding: '1px 6px', marginLeft: 6 }}>{chipForStatus(q.status)}</span>
                      </div>
                  ...
                  {customerInvoices.length ? customerInvoices.map(inv => (
                    <Link key={inv.id} to={`/app/invoices/${inv.id}`} className="ct-history-item">
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        {inv.issued_at && <div className="qb-muted" style={{ fontSize: 'var(--text-2xs)' }}>{formatDate(inv.issued_at)}</div>}
                      </div>
                    </Link>
            ...
            {!selected && !isCreating && (
              <div style={{ padding: '48px 24px', textAlign: 'center', color: 'var(--muted)' }}>
                <div style={{ marginBottom: 8, color: "var(--muted)", display:"flex", justifyContent:"center" }}><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
                <p style={{ fontSize: 'var(--text-base)' }}>Select a contact to view details</p>
      ...
      {showMergeModal && selected && (
        <div className="modal-overlay" onClick={() => setShowMergeModal(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }} role="dialog" aria-modal="true" aria-label="Merge contact">
            <h3 style={{ margin: '0 0 12px', fontSize: 'var(--text-lg)' }}>Merge contact</h3>
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--muted)', marginBottom: 14 }}>
