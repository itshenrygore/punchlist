  ...
  useEffect(() => {
    fetch(`/api/public-quote?token=${shareToken}`)
      .then(async r => {
        ...
        let j; try { j = JSON.parse(text); } catch { throw new Error('Could not reach server — try again in a moment.'); }
        if (!r.ok) {
          if (r.status === 404) throw new Error('This quote link has expired or been removed.');
          if (r.status >= 500) throw new Error('Server issue — try again in a moment.');
          throw new Error(j.error || 'Quote not found');
        }
        return j.quote;
      })
      .then(q => {
