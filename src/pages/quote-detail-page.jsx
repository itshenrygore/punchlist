...
function buildTimeline(quote) {
  ...
  if (quote.signed_at) events.push({ type: 'milestone', icon: '✅', label: 'Approved & signed', time: quote.signed_at });
    ...
    <AppShell title="Quote">
      ...
      <div className="qd-grid">
              ...
              <div className="qd-feed-list">
                {timeline.filter(ev=>ev.type!=='customer_message'&&ev.type!=='contractor_message').map((ev,i) => {
                  ...
                  if((isAm||isAW)&&ev.data){ const d=ev.data; return (
                    <a key={`${ev.subtype}${i}`} href={isAm?`/public/amendment/${d.share_token}`:`/app/additional-work/${d.id}`} target={isAm?'_blank':undefined} rel={isAm?'noreferrer':undefined} className="qd-feed-event qd-feed-event--link"><span className="qd-feed-event-icon">{ev.icon}</span><div className="qd-feed-event-body"><span className="qd-feed-event-label">{ev.label}</span>{d.total&&<span className="qd-feed-event-detail">{currency(d.total)}</span>}</div><div className="qd-feed-event-actions"><StatusBadge status={d.status}/><span className="qd-feed-event-time">{timeAgo(ev.time)}</span></div></a>
                  );}
        ...
        <aside className={`qd-sidebar${mobileTab === 'more' ? ' qd-sidebar--mobile-inline' : ''}${mobileTab !== 'more' ? ' qd-zone-sidebar' : ''}`}>
          {!isDraft && <div className="qb-card"><span className="qb-label">{isLocked?'Share':'Send / Share'}</span>{quote.sent_at&&<div className="qd-sent-pill">Sent {formatDate(quote.sent_at)}</div>}<div className="qd-send-grid">{!isLocked&&quote.customer?.phone&&<button className="btn btn-primary full-width" type="button" disabled={sendingText} onClick={handleSendText}><MessageSquare size={13} style={{verticalAlign:'middle',marginRight:5}}/>{sendingText?'Sending…':quote.sent_at?`Resend to ${quote.customer?.name?.split(' ')[0]||''}` :`Text ${quote.customer?.name?.split(' ')[0]||''}`}</button>}<button className="btn btn-secondary full-width" type="button" onClick={handleCopyLink}><Link2 size={13} style={{verticalAlign:"middle",marginRight:5}}/>{linkCopied?'Copied! ✓':'Copy link'}</button></div><a href={shareUrl+'?preview=1'} target="_blank" rel="noreferrer" className="qd-preview-link">Preview ↗</a></div>}


          <div className="qb-card"><span className="qb-label">Photos {photos.length>0&&`(${photos.length})`}</span>{photos.length>0&&<div className="qd-photos-grid">{photos.map(p=><div key={p.path} className="qd-photo-tile"><img src={p.url} alt="" width={120} height={120} loading="lazy" className="qd-photo-img"/><button type="button" aria-label="Remove photo" onClick={async()=>{try{await deleteQuotePhoto(p.path);setPhotos(pr=>pr.filter(x=>x.path!==p.path));showToast('Removed','info');}catch(e){showToast(friendly(e),'error');}}} className="qd-photo-remove"><X size={12} /></button></div>)}</div>}<label className="qd-photo-upload-label">{photoUploading?'Uploading…':'Add photo'}<input hidden type="file" accept="image/*" multiple onChange={async e=>{const fs=Array.from(e.target.files||[]);if(!fs.length)return;const v=fs.filter(f=>f.size<=5*1024*1024);if(v.length<fs.length)showToast(`${fs.length-v.length} over 5MB`,'error');if(!v.length){e.target.value='';return;}setPhotoUploading(true);let n=0;for(const f of v){try{const p=await uploadQuotePhoto(quoteId,f);setPhotos(pr=>[p,...pr]);n++;}catch(e){showToast(friendly(e),'error');}}setPhotoUploading(false);if(n)showToast(`${n} added`,'success');e.target.value='';}}/></label></div>
