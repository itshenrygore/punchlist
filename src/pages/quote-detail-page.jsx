...
export default function QuoteDetailPage() {
  ...
  useEffect(() => {
    ...
    if (user) getProfile(user.id).then(p => {
      setUserProfile(p);
      // v100 M5: load auto-send invoice pref (default true per §5.5)
      setAutoSendInvoicePref(p?.auto_send_invoice_on_complete !== false);
      // v100 M3: fetch templates for FollowupModal pre-fill
    ...
    getQuote(quoteId)
      .then(data => {
        ...
        if (['completed','invoiced','paid'].includes(data.status)) {
          listInvoices(user?.id).then(invs => { const l = invs.find(i => i.quote_id === quoteId); if (l) setLinkedInvoice(l); }).catch(() => {});
        }
        listAdditionalWork(quoteId).then(setAdditionalWork).catch(() => {});
        listAmendments(quoteId).then(setAmendments).catch(() => {});
        listBookings(user?.id).then(all => setQuoteBookings(all.filter(b => b.quote_id === quoteId))).catch(() => {});
      ...
      <div className="qd-grid">
        <section className="qd-main">
          ...
          <div className="qd-feed">
            <div className="qd-feed-header"><span style={{fontWeight:700,fontSize:"var(--text-base)"}}><MessageSquare size={13} style={{verticalAlign:"middle",marginRight:6}}/>Communication</span><span className="qb-muted fs-12">{(() => { const mc = timeline.filter(e=>e.type==='customer_message'||e.type==='contractor_message').length; return mc > 0 ? `${mc} message${mc !== 1 ? 's' : ''}` : ''; })()}</span></div>
            ...
            {!isDraft && hasShareToken && !['paid','invoiced'].includes(quote?.status) && (
              <div className="qd-feed-reply" style={{marginBottom:12}}><div style={{display:'flex',gap:8,alignItems:'flex-end'}}><textarea value={replyText} onChange={e=>setReplyText(e.target.value)} placeholder={`Reply to ${quote.customer?.name?.split(' ')[0]||'customer'}…`} rows={1} className="qd-feed-reply-input" onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();handleReply();}}} onInput={e=>{e.target.style.height='auto';e.target.style.height=Math.min(e.target.scrollHeight,120)+'px';}} /><button className="btn btn-primary btn-sm" disabled={replySending||!replyText.trim()} onClick={handleReply} style={{flexShrink:0,height:36}}>{replySending?'…':'↩ Reply'}</button></div><div style={{fontSize: 'var(--text-2xs)',color:'var(--muted)',marginTop:4}}>{quote.customer?.email?'Email + quote page':'Quote page only'}{quote.customer?.phone?' + SMS':''}</div></div>
            )}
            {/* Activity timeline */}
            <div className="qd-feed-header" style={{marginTop:8,paddingTop:8,borderTop:'1px solid var(--line)'}}><span style={{fontWeight:600,fontSize: 'var(--text-xs)',color:'var(--muted)'}}>Activity</span><span className="qb-muted" style={{fontSize: 'var(--text-2xs)'}}>{timeline.length} events</span></div>
            {timeline.length===0 && <div style={{padding:'24px 16px',textAlign:'center',color:'var(--muted)',fontSize: 'var(--text-sm)'}}>No activity yet — send the quote to get started.</div>}
            <div className="qd-feed-list">
              {timeline.map((ev,i) => {
                const isMsg = ev.type==='customer_message'||ev.type==='contractor_message';
                const isAm = ev.subtype==='amendment';
                const isAW = ev.subtype==='additional_work';
                if(isMsg&&ev.data){ const e=ev.data; const isCust=e.role==='customer'; return (
                  <div key={`m${i}`} className={`qd-feed-msg ${isCust?'':'qd-feed-msg--right'}`}><div className={`qd-feed-msg-avatar ${isCust?'':'qd-feed-msg-avatar--you'}`}>{isCust?(quote.customer?.name?.trim().split(/\s+/).map(w=>w[0]).slice(0,2).join('').toUpperCase()||'?'):'Me'}</div><div style={{maxWidth:'80%',minWidth:0}}><div className="qd-feed-msg-meta" style={{textAlign:isCust?'left':'right'}}><strong>{isCust?(quote.customer?.name?.split(' ')[0]||'Customer'):'You'}</strong> · {timeAgo(e.timestamp)}</div><div className={`qd-feed-msg-bubble ${isCust?'':'qd-feed-msg-bubble--you'}`}>{e.text}</div>{/* §5.2: read receipt on outbound (contractor) messages */}{!isCust && quote.messages_last_read_at && new Date(quote.messages_last_read_at) > new Date(e.timestamp) && (<div className="qd-feed-msg-read"><Check size={10} style={{verticalAlign:'middle',marginRight:3}}/>Read {timeAgo(quote.messages_last_read_at)}</div>)}</div></div>
                );}
