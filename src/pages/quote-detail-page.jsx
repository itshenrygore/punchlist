...
export default function QuoteDetailPage() {
  ...
  return (
    <AppShell title="Quote">
      ...
      {!isDraft&&!isLocked&&!isRevision&&!isExpired&&hasShareToken&&<div className="qd-mobile-send-bar">{quote.customer?.phone?<button className="btn btn-primary flex-1" type="button" onClick={handleSendText}><MessageSquare size={13} style={{verticalAlign:'middle',marginRight:5}}/>Text {quote.customer?.name?.split(' ')[0] || 'quote'}</button>:<button className="btn btn-primary flex-1" type="button" onClick={handleCopyLink}><Link2 size={13} style={{verticalAlign:"middle",marginRight:5}}/>Copy link</button>}<button className="btn btn-secondary qd-copy-link-btn qd-copy-link-btn-compact" type="button" onClick={handleCopyLink} aria-label="Copy quote link"><Link2 size={14}/><span className="qd-copy-link-text">Link</span></button></div>}
