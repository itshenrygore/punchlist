...
export default function QuoteDetailPage() {
 ...
 {quote.status==='sent' && <div className="qd-phase-banner qd-phase-waiting"><div className="qd-phase-dot qd-dot-sent"></div><div className="qd-phase-body"><div className="qd-phase-title">Sent — waiting on {quote.customer?.name?.split(' ')[0] || 'customer'}</div><div className="qd-phase-hint">{quote.sent_at?`Sent ${formatDate(quote.sent_at)}`:'Sent'}{quote.view_count>0?` · viewed ${quote.view_count}×`:' · not opened yet — a quick text can help'}</div>{/* v100 M3: follow-up context block */}{(() => { const fc=Number(quote.followup_count)||0; const lf=quote.last_followup_at; const vs=Number(quote.views_since_followup)||0; if(fc===0)return null; const daysSince=lf?Math.round((Date.now()-new Date(lf).getTime())/86_400_000):null; const urgColor=daysSince===null?'var(--text-2)':daysSince<2?'var(--green)':daysSince<5?'var(--amber)':'var(--red)'; return <div className="qd-followup-context"><span className="qd-followup-context__stat" style={{color:urgColor}}>Last nudge {daysSince===0?'today':daysSince===1?'1d ago':`${daysSince}d ago`}</span><span className="qd-followup-context__stat">{vs} view{vs!==1?'s':''} since</span></div>; })()}{advice && <div className="qdp-fs-2xs-1715">{advice.emoji} {advice.headline}</div>}</div>{advice&&advice.urgency!=='low' && <button className="btn btn-primary btn-sm shrink-0" type="button" onClick={openNudgeModal}>{quote.customer?.phone ? `Nudge ${quote.customer?.name?.split(' ')[0] || ''}` : 'Send nudge'}</button>}</div>}

 {quote.status==='viewed' && <div className="qd-phase-banner qd-phase-hot"><div className="qd-phase-dot qd-dot-viewed"></div><div className="qd-phase-body"><div className="qd-phase-title">{quote.customer?.name?.split(' ')[0] || 'Customer'} is reviewing{quote.view_count>1?` (${quote.view_count}×)`:''}</div><div className="qd-phase-hint">{showFinancing(quote.total) && quote.view_count > 1 ? `They've looked ${quote.view_count} times — the monthly option (from ${currency(estimateMonthly(quote.total))}/mo) may be what they're considering. A quick text could close this.` : 'They\'re looking at it — a quick text can close this.'}</div>{/* v100 M3: follow-up context block */}{(() => { const fc=Number(quote.followup_count)||0; const lf=quote.last_followup_at; const vs=Number(quote.views_since_followup)||0; if(fc===0)return null; const daysSince=lf?Math.round((Date.now()-new Date(lf).getTime())/86_400_000):null; const urgColor=daysSince===null?'var(--text-2)':daysSince<2?'var(--green)':daysSince<5?'var(--amber)':'var(--red)'; return <div className="qd-followup-context"><span className="qd-followup-context__stat" style={{color:urgColor}}>Last nudge {daysSince===0?'today':daysSince===1?'1d ago':`${daysSince}d ago`}</span><span className="qd-followup-context__stat">{vs} view{vs!==1?'s':''} since</span></div>; })()}</div><button className="btn btn-primary btn-sm shrink-0" type="button" onClick={openNudgeModal}>{quote.customer?.phone ? `Nudge ${qd-phase-dot qdp-s32-0697er?.name?.split(' ')[0] || ''}` : 'Send nudge'}</button></div>}
 ...
 {(quote.assumptions||quote.exclusions)&&<div className="qd-two-col" >{quote.assumptions&&<div className="qdp-fs-xs-1f78"><strong className="qdp-block_fs-2xs-05bf">Assumptions</strong>{quote.assumptions}</div>}{quote.exclusions&&<div className="qdp-fs-xs-1f78"><strong className="qdp-block_fs-2xs-05bf"ginBottom:4}}>Not included</strong>{quote.exclusions}</div>}</div>}
...
function ItemModal({ title, subtitle, signedInfo, modeHint, draft, setDraft, saving, province, country, reasonLabel, reasonPlaceholder, onSaveDraft, onSend, onClose }) {
 ...
 <div className="qdp-flex-ebc1"><h2 className="qdp-s8-e829"fs-xs-e749em'}}>{title}</h2><button className="btn btn-secondary btn-sm" type="button" onClick={onClose} aria-label="Close"><X size={14} /></button></div>
 <div className="qb-muted" >{subtitle}</div>
 {signedInfo && <div className="qdp-fs-xs-203b">{signedInfo}</div>}
 {modeHint && <div className="qdp-fs-2xs-2540"ineHeight:1.5}}>ℹ {modeHint}</div>}
