        ...
        <section className="qd-main">
          ...
          <div className={`qd-hero${mobileTab !== 'details' ? ' qd-hero--compact' : ''}`}>
           {mobileTab === 'details' ? (<>
            ...
            {quote.status==='sent' && <div className="qd-phase-banner qd-phase-waiting"><div className="qd-phase-dot qd-dot-sent"></div><div className="qd-phase-body"><div className="qd-phase-title">Sent — waiting on {quote.customer?.name?.split(' ')[0] || 'customer'}</div><div className="qd-phase-hint">{quote.sent_at?`Sent ${formatDate(quote.sent_at)}`:'Sent'}{quote.view_count>0?` · viewed ${quote.view_count}×`:' · not opened yet — a quick text can help'}</div>{/* v100 M3: follow-up context block */}{(() => { const fc=Number(quote.followup_count)||0; const lf=quote.last_followup_at; const vs=Number(quote.views_since_followup)||0; if(fc===0)return null; const daysSince=lf?Math.round((Date.now()-new Date(lf).getTime())/86_400_000):null; const urgColor=daysSince===null?'var(--text-2)':daysSince<2?'var(--green)':daysSince<5?'var(--amber)':'var(--red)'; return <div className="qd-followup-context"><span className="qd-followup-context__stat" style={{color:urgColor}}>Last follow-up {daysSince===0?'today':daysSince===1?'1d ago':`${daysSince}d ago`}</span><span className="qd-followup-context__stat">{vs} view{vs!==1?'s':''} since</span></div>; })()}{advice && <div style={{marginTop:4,fontSize: 'var(--text-2xs)',color:'var(--text-2)'}}>{advice.emoji} {advice.headline}</div>}</div>{<button className="btn btn-secondary btn-sm shrink-0" type="button" onClick={openNudgeModal}>{quote.customer?.phone ? `Follow up with ${quote.customer?.name?.split(' ')[0] || ''}` : 'Send follow-up'}</button>}</div>}

            {quote.status==='viewed' && <div className="qd-phase-banner qd-phase-hot"><div className="qd-phase-dot qd-dot-viewed"></div><div className="qd-phase-body"><div className="qd-phase-title">{quote.customer?.name?.split(' ')[0] || 'Customer'} is reviewing{quote.view_count>1?` (${quote.view_count}×)`:''}</div><div className="qd-phase-hint">{quote.view_count > 1 ? `They've looked ${quote.view_count} times — they're weighing it. A quick text could close this.` : 'They\'re looking at it — a quick text can close this.'}</div>{/* v100 M3: follow-up context block */}{(() => { const fc=Number(quote.followup_count)||0; const lf=quote.last_followup_at; const vs=Number(quote.views_since_followup)||0; if(fc===0)return null; const daysSince=lf?Math.round((Date.now()-new Date(lf).getTime())/86_400_000):null; const urgColor=daysSince===null?'var(--text-2)':daysSince<2?'var(--green)':daysSince<5?'var(--amber)':'var(--red)'; return <div className="qd-followup-context"><span className="qd-followup-context__stat" style={{color:urgColor}}>Last follow-up {daysSince===0?'today':daysSince===1?'1d ago':`${daysSince}d ago`}</span><span className="qd-followup-context__stat">{vs} view{vs!==1?'s':''} since</span></div>; })()}</div><button className="btn btn-secondary btn-sm shrink-0" type="button" onClick={openNudgeModal}>{quote.customer?.phone ? `Follow up with ${quote.customer?.name?.split(' ')[0] || ''}` : 'Send follow-up'}</button></div>}
            ...
            {isExpiringSoon && !isExpired && !['sent', 'viewed', 'revision_requested', 'approved', 'approved_pending_deposit', 'deposit_paid', 'converted_to_invoice', 'paid'].includes(quote.status) && (
          ...
          <div className={`qd-scope${mobileTab !== 'details' ? ' qd-zone-details' : ''}`}>
            ...
            {scopeOpen && <div className="qd-scope-body">
              ...
              {(() => {
                const computedSubtotal = (quote.line_items||[]).filter(i=>i.included!==false).reduce((s,i)=>s + Number(i.quantity||0)*Number(i.unit_price||0), 0);
                const sub = Number(quote.subtotal) > 0 ? Number(quote.subtotal) : computedSubtotal;
                const tot = Number(quote.total) || 0;
                const tax = Number(quote.tax) > 0 ? Number(quote.tax) : Math.max(0, tot - sub - Number(quote.discount||0));
                return (
                  <div className="qd-totals"><div className="qb-total-row"><span>Subtotal</span><span>{currency(sub)}</span></div>{Number(quote.discount||0)>0&&<div className="qb-total-row"><span>Discount</span><span>−{currency(quote.discount)}</span></div>}<div className="qb-total-row"><span>Tax</span><span>{currency(tax)}</span></div><div className="qb-total-row grand"><span>Total</span><span>{currency(tot)}</span></div>{quote.deposit_required&&Number(quote.deposit_amount)>0&&<div className="qb-total-row qd-deposit-row"><span>Deposit ({labelForDeposit(quote.deposit_status)})</span><span>{currency(quote.deposit_amount)}</span></div>}</div>
                );
              })()}
