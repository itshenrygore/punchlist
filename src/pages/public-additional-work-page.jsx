...
export default function PublicAdditionalWorkPage() {
  ...
  if (error && !request) return (
    <div className="marketing-shell">
      <main className="container" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, textAlign: 'center' }}>
        <div style={{ fontSize: '2rem' }}>🔗</div>
        <h2>This request isn't available</h2>
  ...
  const statusBanner = isApproved
    ...
    : isNeedsReview
    ? { bg: 'var(--blue-bg)', border: 'rgba(72,120,208,.2)', icon: '💬', color: 'var(--blue)', title: 'Question sent', sub: 'Your contractor will reply to your question.' }
    : isDeclined
    ? { bg: 'var(--red-bg)', border: 'rgba(180,35,24,.15)', icon: '✕', color: 'var(--red)', title: 'Additional work declined', sub: 'This extra work will not be performed.' }
    : null;
    ...
    <div className="marketing-shell">
      ...
      <main className="container public-quote-layout">
        <section className="stack-lg">
          ...
          <div className="panel" style={{ display: 'flex', gap: 12, padding: '14px 18px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 120 }}>
              ...
              <div style={{ fontWeight: 700, color: isApproved ? 'var(--green)' : isDeclined ? 'var(--red)' : 'var(--amber)', fontSize: 13 }}>
        ...
        <aside className="stack-lg public-actions-sidebar">
          ...
          <div className="panel sticky-panel">
            ...
            <div className="stack">
                ...
                <>
                  ...
                  {mode === 'question' && (
                    <div className="stack" style={{ marginTop: 4 }}>
                      <textarea className="input textarea-md" value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="What would you like to know? Your contractor will reply directly." rows={3} autoFocus />
              ...
              {canAct && !isNeedsReview && (
                <>
                  <button className="btn btn-secondary full-width" type="button" style={{ color: 'var(--muted)', fontSize: 12, marginTop: 4 }} onClick={() => { setMode(mode === 'decline' ? null : 'decline'); setFeedback(''); }}>
              ...
              {error && <div style={{ color: 'var(--red)', fontSize: 13, fontWeight: 600, padding: '6px 0' }}>{error}</div>}
            </div>
          </div>
        ...
        {(isApproved || isDeclined || isNeedsReview) && (
          <div className="public-actions-mobile-bar">
            <div style={{ padding: '8px 0', fontWeight: 700, color: 'var(--muted)', fontSize: 13, textAlign: 'center', flex: 1 }}>
