    ...
    <header className={`lp-nav ${sc ? 'lp-nav--scrolled' : ''}`}>
      <div className="lp-nav-inner" style={{ height: sc ? 56 : 64 }}>
        <Link to="/" className="lp-brand">
          <div className="lp-brand-mark"><Zap size={15} color="#fff" strokeWidth={2.5} /></div>
        ...
        <button className="lp-mob-btn" onClick={() => setOp(!op)}>{op ? <X size={22} /> : <Menu size={22} />}</button>
      </div>
      {op && (
    ...
    <section className="lp-hero">
      <div className="lp-container">
        <div className="lp-hero-grid">
          <div className="lp-hero-copy" style={{ opacity: ok ? 1 : 0, transform: ok ? 'none' : 'translateY(24px)' }}>
          ...
          <div className="lp-hero-visual" style={{ opacity: ok ? 1 : 0, transform: ok ? 'none' : 'translateY(32px)' }}>
            <HeroScene active={ok} />
          </div>
        </div>
      </div>
...
function HeroScene({ active }) {
  ...
  return (
    <div className="lp-scene-wrap">
      <div className="lp-hero-chips" style={{ position:'absolute', top:-4, right:-8, zIndex:10, animation: active ? 'lpFloat1 4s ease-in-out infinite' : 'none' }}><Chip icon={<Eye size={12}/>} text="Lisa viewed your quote" color="var(--lp-blue)" show={active} delay="0.6s"/></div>
      <div className="lp-hero-chips" style={{ position:'absolute', top:'48%', right:-20, zIndex:10, animation: active ? 'lpFloat2 5s ease-in-out infinite 0.8s' : 'none' }}><Chip icon={<CheckCircle size={12}/>} text="Approved + signed" color="var(--lp-green)" show={active} delay="1.8s"/></div>
      <div className="lp-hero-chips" style={{ position:'absolute', bottom:48, left:-12, zIndex:10, animation: active ? 'lpFloat1 4.5s ease-in-out infinite 1.2s' : 'none' }}><Chip icon={<DollarSign size={12}/>} text="Paid in full" color="var(--lp-accent)" show={active} delay="3s"/></div>
      <div className="card-lift lp-quote-card">
        ...
        <div className="lp-quote-content" style={{ opacity: ph==='out'?0:1, transform: ph==='out'?'translateY(6px)':'none' }}>
          ...
          <div className="lp-line-items">
            {job.items.map((item, i) => (
              <div key={i} className="lp-line-item" style={{ animation: active && ph==='show' ? `lpFadeUp 0.4s var(--lp-ease) ${0.1+i*0.08}s both` : 'none' }}>
...
function Chip({ icon, text, color, show, delay='0s' }) {
  return (
    <div className="lp-chip" style={{ opacity:show?1:0, transform:show?'scale(1)':'scale(0.9)', transitionDelay:delay }}>
      <div className="lp-chip-icon" style={{ background:`${color}12`, color }}>{icon}</div>
      <span>{text}</span>
    </div>
  );
}
...
function OC({ num, title, desc, visual, accent }) {
  return (
    <div className="card-lift lp-outcome-card">
      <span className="lp-outcome-num" style={{ color:accent }}>{num}</span>
...
function CloseV() {
  return (
    <div className="lp-close-visual">
      <div className="lp-pc lp-pc--old"><div className="lp-pc-label">Full price</div><div className="lp-pc-amount" style={{textDecoration:'line-through',opacity:0.35}}>$6,000</div><div className="lp-pc-react" style={{color:'var(--red)'}}>"Let me think about it"</div></div>
      <ArrowRight size={14} style={{ color:'var(--lp-text-3)',flexShrink:0 }}/>
      <div className="lp-pc lp-pc--new"><div className="lp-pc-label" style={{color:'var(--lp-accent-deep)'}}>Monthly</div><div className="lp-pc-amount lp-accent-text">$500/mo</div><div className="lp-pc-react" style={{color:'var(--lp-green)'}}>"When can you start?"</div></div>
    </div>
  );
}
function TrackV() {
  return <div>{[{d:'var(--lp-blue)',t:'Lisa viewed quote',tm:'2m ago'},{d:'var(--lp-purple)',t:'Viewed (3rd time)',tm:'1h ago'},{d:'var(--lp-accent)',t:'Follow-up sent',tm:'3h ago'}].map((e,i) => (
    <div key={i} className="lp-track-ev" style={{borderBottom:i<2?'1px solid var(--lp-border)':'none'}}>
...
function PaidV() {
  return (<div style={{textAlign:'center'}}><div className="lp-paid-badge"><CheckCircle size={15} className="lp-green-icon"/><span>$4,847 deposited</span></div><div className="lp-paid-sub">Customer pays monthly · You get paid now</div></div>);
}
...
function LpStickerShock() {
  ...
  return (
    <section ref={ref} className="lp-section lp-section--warm">
      <div className="lp-container lp-shock-wrap">
        <Rv>
          <span className="lp-eyebrow">The real problem</span>
          <h2 className="lp-h2" style={{marginBottom:14}}>The quotes you lose<br/>aren't bad quotes.</h2>
        ...
        <Rv delay={150}>
          <div className="lp-shock-container">
            <div className="lp-shock-card" style={{opacity:flipped?0:1,transform:flipped?'scale(0.96) translateY(-6px)':'scale(1)',filter:flipped?'blur(3px)':'blur(0)'}}>
              <div className="lp-shock-amount" style={{opacity:0.3,textDecoration:'line-through'}}>$6,000</div>
              <div className="lp-shock-label">Customer sees the total</div>
              <div className="lp-shock-reaction" style={{color:'var(--red)'}}>"I need to think about it."</div>
            </div>
            <div className="lp-shock-card lp-shock-card--accent" style={{opacity:flipped?1:0,transform:flipped?'scale(1)':'scale(0.96) translateY(6px)',filter:flipped?'blur(0)':'blur(3px)'}}>
              <div className="lp-shock-amount lp-accent-text">~$500/mo</div>
              <div className="lp-shock-label">Same job, monthly option</div>
              <div className="lp-shock-reaction" style={{color:'var(--lp-green)'}}>"When can you start?"</div>
            </div>
          </div>
        </Rv>
        <Rv delay={300}>
...
function LpHowItWorks() {
  return (
    <section id="how" className="lp-section lp-section--dark">
      ...
      <div className="lp-container" style={{maxWidth:900,position:'relative',zIndex:1}}>
        <Rv><div className="lp-section-head"><span className="lp-eyebrow">How it works</span><h2 className="lp-h2" style={{color:'#fff'}}>You know the job.<br/>We handle the quote.</h2></div></Rv>
      ...
      <div className="lp-container lp-foreman-grid">
        <Rv>
          <div className="card-lift lp-foreman-card">
            ...
            {[{icon:<Bell size={14} style={{color:'var(--lp-accent)'}}/>,bg:'var(--lp-accent-light)',t:'You may be missing venting',sub:'93% of similar jobs include this'},{icon:<TrendingUp size={14} style={{color:'var(--lp-blue)'}}/>,bg:'rgba(53,115,226,0.08)',t:'Typical range: $4,200 – $5,400',sub:'Based on Alberta pricing data'},{icon:<CheckCircle size={14} style={{color:'var(--lp-green)'}}/>,bg:'var(--lp-green-light)',t:'Add: Permit + inspection — $250',sub:'Required for gas appliance work'}].map((item,i) => (
              <div key={i} className="lp-foreman-item" style={{borderBottom:i<2?'1px solid var(--lp-border)':'none'}}>
                <div className="lp-foreman-item-icon" style={{background:item.bg}}>{item.icon}</div>
                <div style={{minWidth:0}}><div className="lp-foreman-item-title">{item.t}</div><div className="lp-foreman-item-sub">{item.sub}</div></div>
              </div>
            ))}
          </div>
        </Rv>
        <Rv delay={80}>
          <div>
            <span className="lp-eyebrow" style={{color:'var(--lp-purple)'}}>Intelligence layer</span>
            <h2 className="lp-h2" style={{marginBottom:14}}>Foreman catches what<br/>you might miss.</h2>
  ...
  return (
    <section id="pricing" className="lp-section">
      <div className="lp-container" style={{maxWidth:800}}>
        <Rv><div className="lp-section-head"><span className="lp-eyebrow">Pricing</span><h2 className="lp-h2" style={{marginBottom:12}}>One extra closed job<br/>pays for the year.</h2><p className="lp-pricing-sub">Start free, upgrade when it's working.</p></div></Rv>
        <Rv delay={50}>
          <div className="lp-billing-toggle-wrap">
            <div className="lp-billing-toggle">
              {[{k:false,l:'Monthly'},{k:true,l:'Yearly',b:'Save $99'}].map(b => (
                <button key={String(b.k)} onClick={() => setYr(b.k)} className={`lp-billing-btn ${yr===b.k?'lp-billing-btn--on':''}`}>{b.l}{b.b && <span className="lp-save-badge">{b.b}</span>}</button>
              ))}
            </div>
          </div>
        </Rv>
        <div className="lp-pricing-grid">
          ...
          <Rv delay={180}>
            <div className="lp-price-card lp-price-card--pro">
              ...
              {['Everything in Free','Unlimited quotes','Customer financing (paid in full)','Real-time tracking + notifications','Follow-up prompts','Deposit collection via Stripe','Invoicing + scheduling'].map(f => <div key={f} className="lp-feature-row"><CheckCircle size={15} style={{color:'var(--lp-accent)',flexShrink:0}}/> {f}</div>)}
              <Link to="/signup" className="btn-glow lp-price-cta-pro">Start with Pro →</Link>
              <div className="lp-price-micro">Cancel anytime · No contracts</div>
            </div>
          </Rv>
        </div>
      </div>
...
function LpFaq() {
  ...
  const items = [
    ...
    ['Is my data secure?','Stored on Supabase with row-level security. All connections encrypted. We never share your data.'],
  ];
  return (
          ...
          {items.map(([q,a],i) => (
            <Rv key={i} delay={i*40}>
              <div className={`lp-faq-item ${op===i?'lp-faq-item--open':''}`}>
                <button onClick={() => setOp(op===i?null:i)} className="lp-faq-q"><span>{q}</span><ChevronDown size={16} className="lp-faq-chevron" style={{transform:op===i?'rotate(180deg)':'rotate(0)'}}/></button>
                <div className="lp-faq-a" style={{maxHeight:op===i?300:0,opacity:op===i?1:0}}><div className="lp-faq-a-inner">{a}</div></div>
              </div>
            </Rv>
          ))}
        </div>
...
function LpFinalCTA() {
  return (
    <section className="lp-final-cta">
      <div className="lp-glow-orb" style={{top:'10%',left:'20%',width:400,height:400}}/>
      <div className="lp-container" style={{maxWidth:600,textAlign:'center',position:'relative',zIndex:1}}>
        <Rv>
          <h2 className="lp-final-h2">Your next quote<br/>should work harder.</h2>
          <p className="lp-final-sub">Build it in minutes. Know when they open it. Let them pay monthly. Close the job — and get paid in full.</p>
          <div className="lp-final-btns">
            <Link to="/signup" className="btn-glow lp-btn-primary" style={{fontSize: 'var(--text-lg)',padding:'15px 30px'}}>Send your first quote free <ArrowRight size={17}/></Link>
  ...
  return (
    <footer className="lp-footer">
      <div className="lp-container lp-footer-inner">
        <div>
          <div className="lp-brand" style={{marginBottom:5}}><div className="lp-brand-mark" style={{width:24,height:24}}><Zap size={12} color="#fff" strokeWidth={2.5}/></div><span className="lp-brand-text" style={{fontSize: 'var(--text-md)'}}>punchlist</span></div>
