...
export default function InvoiceDetailPage() {
  ...
  async function handleSendText() {
    ...
    const smsBody = encodeURIComponent(
      `Hi${firstName ? ' ' + firstName : ''}, your invoice from ${compName} for ${currency(balance)} is ready: ${url}`
    );
  ...
  if (loading) return <AppShell title="Invoice"><div className="loading-inline"><div className="loading-spinner"/>Loading…</div></AppShell>;
      ...
      <div className="inv-layout">
          ...
          {editing ? (
            <div>
              ...
              <div style={{ marginBottom: 12 }}>
                <div className="inv-edit-header" style={{ display: 'grid', gap: 6, padding: '6px 0', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--muted)' }}>
                ...
                {editItems.map((item, idx) => (
                  <div key={item.id || idx} className="inv-edit-row" style={{ display: 'grid', gap: 6, padding: '6px 0', borderBottom: '1px solid var(--line)', alignItems: 'center' }}>
                    <input className="input" style={{ fontSize: 12, padding: '6px 8px', minWidth: 0 }} value={item.name} onChange={e => updateEditItem(idx, 'name', e.target.value)} placeholder="Item name" />
                    <input className="input" style={{ fontSize: 12, padding: '6px 8px', textAlign: 'right' }} type="number" min="0" step="0.25" value={item.quantity} onChange={e => updateEditItem(idx, 'quantity', e.target.value)} />
                    <input className="input" style={{ fontSize: 12, padding: '6px 8px', textAlign: 'right' }} type="number" min="0" step="1" value={item.unit_price} onChange={e => updateEditItem(idx, 'unit_price', e.target.value)} />
                    <button className="btn btn-secondary btn-sm" type="button" onClick={() => removeEditItem(idx)} style={{ padding: '4px 6px', fontSize: 11, color: 'var(--red)' }}>✕</button>
                  </div>
                ))}
                <button className="btn btn-secondary btn-sm" type="button" onClick={addEditItem} style={{ marginTop: 8, fontSize: 11 }}>+ Add item</button>
              </div>
              {editTotals && (
            ...
            <>
              ...
              <div className="inv-items">
                ...
                {hasAdditionalWork ? (
                  Object.entries(groupedItems).map(([group, groupItems]) => (
                    <div key={group}>
                      <div style={{ padding: '10px 0 4px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: group === 'Additional Work' ? 'var(--amber)' : 'var(--muted)', borderLeft: group === 'Additional Work' ? '3px solid var(--amber)' : 'none', paddingLeft: group === 'Additional Work' ? 10 : 0 }}>
              ...
              {payments.length > 0 && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--line)' }}>
                  <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--muted)', marginBottom: 8 }}>Payment History</div>
                  {payments.map(p => (
                    <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: '1px solid var(--line)', fontSize: 12 }}>
                      <div>
                      ...
                      {!isPaid && (
                        <button className="btn btn-secondary btn-sm inv-payment-delete" type="button" onClick={() => handleDeletePayment(p.id)} style={{ padding: '2px 6px', fontSize: 10, color: 'var(--red)' }}>✕</button>
                      )}
                    </div>
                  ))}
                </div>
        ...
        <aside className="inv-sidebar">
          <div className="qb-card">
            ...
            {isPartial && !isPaid && (
              <div style={{ color: 'var(--amber)', fontSize: 12, fontWeight: 700, marginTop: 8 }}>
                ...
                <>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {invoice.customer?.email && (
                      <button className="btn btn-primary" type="button" style={{ flex: 1 }} onClick={handleSendEmail}>
                        ✉ {['sent','viewed','partial','overdue'].includes(invoice.status) ? 'Resend' : 'Email'}
                      </button>
                    )}
                  ...
                  <button className="btn btn-secondary full-width" type="button" onClick={handleSendCopy}>🔗 Copy link</button>
                  ...
                  {!showPartialForm && !showPayForm ? (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 4 }}>
                      <button className="btn btn-secondary full-width" type="button" onClick={() => setShowPartialForm(true)} style={{ fontSize: 11 }}>
                        Record payment
                      </button>
                      <button className="btn btn-secondary full-width" type="button" onClick={() => setShowPayForm(true)} style={{ color: 'var(--green)', fontSize: 11 }}>
                        Mark fully paid
                      </button>
                    </div>
                  ) : showPayForm ? (
                    ...
                    <div className="inv-pay-form">
                      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>Record Payment</div>
          ...
          {profile?.payment_instructions && (
            <div className="qb-card">
              <div style={{ padding: '8px 10px', background: 'var(--bg)', borderRadius: 'var(--r-sm)', fontSize: 11, color: 'var(--muted)' }}>
                <strong style={{ display: 'block', marginBottom: 4, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.06em' }}>Payment instructions (visible to customer)</strong>
                {profile.payment_instructions}
              </div>
            </div>
          )}
          ...
          {!isPaid && (
            <div className="qb-card">
              ...
              <p style={{ margin: '4px 0 10px', fontSize: 11, color: 'var(--muted)' }}>
                Send automatic email reminders when overdue
              </p>
              {REMINDER_OPTIONS.map(opt => (
                <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', fontSize: 12, cursor: 'pointer', color: 'var(--text-2)' }}>
                  <input
                    type="checkbox"
