...
export default function InvoiceDetailPage() {
  ...
  async function handleRecurringChange(interval) {
    await setRecurringInterval(invoice.id, interval);
    setInvoice(p => ({ ...p, recurring_interval: interval || null }));
    ...
    <AppShell title={`Invoice ${invoice.invoice_number || ''}`} actions={
      ...
      <div className="inv-layout">
        ...
        <div className="inv-doc">
          {editing ? (
              ...
              <div className="id-edit-items">
                ...
                {editItems.map((item, idx) => (
                  <div key={item.id || idx} className="inv-edit-row id-edit-item-row">
                    ...
                    <button className="btn btn-secondary btn--xs id-edit-remove" type="button" onClick={() => removeEditItem(idx)} aria-label="Remove item"><X size={12} /></button>
                  </div>
                ))}
            ...
            <>
              ...
              {isOverdue && <div className="inv-overdue-banner">{daysOverdue} day{daysOverdue !== 1 ? 's' : ''} overdue{lateFee > 0 ? ` · ${currency(lateFee)} late fee applies` : ''}</div>}
              ...
              <div className="inv-totals">
                ...
                {lateFee > 0 && <div className="inv-total-row id-color-red"><span>Late fee ({profile?.late_fee_percent}%)</span><span>+{currency(lateFee)}</span></div>}
                <div className="inv-total-row inv-grand"><span>Total</span><span>{currency(Number(invoice.total || 0) + lateFee)}</span></div>
                {totalPaid > 0 && !isPaid && (
                  <>
                ...
                <div className="id-pay-history">
                  ...
                  {payments.map(p => (
                    <div key={p.id} className="id-pay-row">
                      ...
                      {!isPaid && (
                        <button className="btn btn-secondary btn-sm inv-payment-delete id-pay-delete" type="button" onClick={() => handleDeletePayment(p.id)} aria-label="Delete payment">×</button>
                      )}
                    </div>
                  ))}
                </div>
        ...
        <aside className="inv-sidebar">
          ...
          <div className="qb-card">
            <span className="qb-label">Recurring</span>
            <p className="id-reminder-desc">Auto-create next invoice when this one is paid</p>
            <select
              className="input"
