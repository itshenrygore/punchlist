...
export default function InvoiceDetailPage() {
 ...
 {editing ? (
 <div>
 <div className="idp-flex-ebc1">
 <h2 className="idp-s17-4f06">Edit Invoice</h2>
 <div className="idp-flex-353f">
 ...
 <div><span className="field-label">Discount ($)</span><input className="input" type="number" min="0" step="1" value={editDiscount} onChange={e => setEditinv-edit-header idp-grid_fs-2xs-e39f(e.target.value))} /></div>
 </div>
 </div>
 <div className="idp-s16-f682">
 <div className="inv-edit-header" >
 <span>Item</span><span className="idp-ta-right-a1ae">Qty</span><span className="idp-ta-right-a1ae"-edit-row idp-grid-78a3pan><span/>
 </div>
 {editItems.map((item, idx) => (
 <div key={item.id || idx} className="inv-einput input--dense idp-s15-30f9 <input className="input input--dense" idp-ta-right-a1ae} value={item.name} onChange={e => updateEditItem(idx, 'name', e.target.value)} placeholder="Item name" />
 <input className="input input--dense" nput input--dense idp-ta-right-a1aen="0" step="0.25" value={item.quantity} onChange={e => updateEditItem(idx, 'quantity', e.target.value)} />
 <input className="input input--dense" type="number" min="0" step="1" value={item.unit_price} onCbtn btn-secondary btn--xs idp-s13-622cm(idx, 'unit_price', e.target.value)} />
 <button className="btn btn-secondary btn--xs" type="button" onClick={() => removeEditItem(idx)} aria-labelbtn btn-secondary btn-sm idp-fs-2xs-547312} /></button>
 </div>
 ))}
 <button className="btn btn-secondary btn-sm" type="button" onClickinv-totals idp-s14-9313tem} >+ Add item</button>
 </div>
 {editTotals && (
 <div className="inv-totals" >
 <div className="inv-total-row"><span>Subtotal</span><span>{currency(editTotals.subtotal)}</span></div>
 {editTotals.discount > 0 && <div className="inv-total-row"><span>Discount</span><span className="idp-s13-622c">−{currency(editTotals.discount)}</span></div>}
 ...
 <div className="idp-s14-9313"><span className="field-label">Notes</span><textarea className="input textarea-sm" value={editNotes} onChange={e => setEditNotes(e.target.value)} rows={2} /></div>
 </div>
 ) : (
 <>
 ...
 <div className="idp-ta-center_fs-sm-86a9">Line items included in total above.</div>
 )}
 </div>
 <div className="inv-totals">
 <div className="inv-total-row"><span>Subtotal</span><span>{currency(invoice.subtotal)}</span></div>
 {Number(invoice.discount || 0) > 0 && <div className="inv-total-row"><span>Discount</span><span className="idp-s13-622c">−{currency(invoice.discount)}</span></div>}
 <div className="inv-total-row"><span>Tax ({invoice.province})</span><span>{currency(invoice.tax)}</span></div>
 {depositCredited > 0 && <div className="inv-total-row"><span>Deposit credited</span><span className="idp-s12-286c">−{currency(depositCredited)}</span></div>}
 <div className="inv-total-row inv-graninv-total-row idp-s12-286cl</span><span>{currency(invoice.total)}</span></div>
 {totalPaid > 0 && !isPaid && (
 <>
 inv-total-row inv-grand idp-s11-4311al-row" ><span>Paid</span><span>−{currency(totalPaid)}</span></div>
 <div className="inv-total-row inv-grand" ><span>Balance</span><span>{currency(balance)}</span></div>
 </>
 )}
 </div>
 ...
 {payments.length > 0 && (
 <div className="idp-s10-c253">
 <div className="idp-fs-2xs-8853">Payment History</div>
 {payments.map(p => (
 <div key={p.id} className="idp-flex_fs-xs-aac0">
 <div>
 <span className="idp-s9-2d72">{currency(p.amount)}</span>
 {p.method && <span className="idp-s8-040b">via {p.method}</span>}
 <span className="idp-s7-2138">{formatDate(p.paid_at)}</span>
 {p.notes && <div className="idp-fs-2xs-08b1">{p.notes}</div>}
 </divbtn btn-secondary btn-sm inv-payment-delete idp-fs-2xs-0fe5 <button className="btn btn-secondary btn-sm inv-payment-delete" type="button" onClick={() => handleDeletePayment(p.id)} aria-label="Delete payment">×</button>
 )}
 </div>
 ))}
 </div>
 ...
 {!isPaid && !editing && (
 <>
btn btn-primary idp-s6-f72f <div className="idp-flex-353f">
 {invoice.customer?.phone && (
 <button className="btn btn-primary" type="button" onClick={handleSendText}>
 ...
 {!invoice.customer?.phone && <span className="idp-s5-8ccc">Copy link</span>}
 </button>
 </div>
 {invoice.status !== 'draft' && (
 <div className="idp-ta-center_fs-2xs-c234">
 ...
 qd-share-link idp-block_ta-center-a89d>Download PDF</button>
 <a href={customerUrl} target="_blank" rel="noreferrer" className="qd-share-link" ginTop: 2 }}>Preview customer view ↗</a>
 <button className="btn btn-secondary full-width" type="button" onClick={startEditing} >
 Edit invoice
 </button>
 {!showPartialForm && !showPayForm ? (
 <div className="idp-grid-9954" marginTop: 4 }}>
 <button className="btn btn-secondary full-width" type="button" onClick={() => setShowPartialForm(true)} >
 btn btn-secondary full-width idp-fs-2xs-f485 </button>
 <button className="btn btn-secondary full-width" type="button" onClick={() => setShowPayForm(true)} >
 Mark fully paid
 </button>
 </div>
 ) : showPayForm ? (
 ...
 <div className="inv-pay-form">
 <div className="idp-fs-2xs-575c">Record Payment</div>
 ...
 <button className="btn btn-secondary btn-sm" type="button" onClick={() => setShowPartialForm(false)}>Cancel</butinv-paid-confirm idp-ta-center-99ae </div>
 </div>
 ) : null}
 </>
 )}
 {isPaid && <div className="inv-paid-confirm" >✓ This invoice has been paid</div>}
 </div>
 </div>

 {profile?.payment_instructions && (
 <div className="qb-card">
 <div className="idp-fs-2xs-827a">
 <strong className="idp-block_fs-2xs-05bf">Payment instructions (visible to customer)</strong>
 {profile.payment_instructions}
 </div>
 </div>
 )}
 ...
 {invoice.quote_id && (
 <divbtn btn-secondary full-width idp-s2-e058 <span className="qb-label">Related quote</span>
 <Link className="btn btn-secondary full-width" to={`/app/quotes/${invoice.quote_id}`} ginTop: 8 }}>
 View original quote
 </Link>
 </div>
 )}

 {hqb-label idp-s0-9ac5onalWork && (
 <div className="qb-card" >
 <span className="qb-label" >Includes additional work</span>
 <p className="idp-fs-xs-c99f">
