...
export default function QuotesListPage() {
  ...
  const [statusFilter, setStatusFilter]   = useState(() => {
    const valid = ['draft','sent','viewed','approved','scheduled','completed','invoiced','paid','declined','expired','needs_followup'];
  ...
  const filtered = useMemo(() => {
    ...
    if (statusFilter === 'needs_followup') {
      result = result.filter(q => ['viewed','question_asked'].includes(q.status) || (q.status === 'sent' && q.view_count > 0));
