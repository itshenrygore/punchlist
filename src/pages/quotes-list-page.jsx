...
function QuoteRow({ quote }) {
 ...
 const viewBadge = quote.view_count > 0 && ['sent'ql-row qlp-grid-680fed','question_asked'].includes(quote.status);
 return (
 <Link
 ...
 >
 <span className="qlp-fs-sm-7f3b">{num}</span>
 <span className="qlp-fs-base-4c16">
 {quote.title || 'Untitled quote'}
 {quote.customer?.name && (
 <span className="qlp-fs-sm-c0c1">· {quote.customer?.name}</span>
 )}
 </span>
 <span className="qlp-flex-aa31">
 <StatusBadge status={quote.status} />
 {viewBadge && <span className="ql-view-badge">{quote.view_count}×</span>}
 </span>
 <span className="qlp-ta-right_fs-base-8019">
 {currency(quote.total || 0)}
 </span>
 <span className="qlp-ta-right_fs-xs-628a">
...
function QuoteCard({ quote }) {
 ...
 const viewBadge = quoteql-card-v2 qlp-s6-bd50t > 0 && ['sent','viewed','question_asked'].includes(quote.status);
 return (
 <Link to={`/app/quotes/${quote.id}`} className="ql-card-v2" >
 <div className="qlp-flex-f55b">
 <div className="qlp-s3-d14f">
 <div className="ql-card-title">{quote.title || 'Untitled quote'}</div>
 <div className="ql-card-meta">
 <span>{quote.customer?.name || <span className="qlp-s5-8ec5">No contact</span>}</span>
 {num && <span className="qlp-s4-07e6">· {num}</span>}
 </div>
 </div>
 <div className="ql-card-total">{currency(quote.total || 0)}</div>
 </div>
 <div className="ql-card-bottom">
 <div className="qlp-flex-4579">
 <StatusBadge status={quote.status} />
 {viewBadge && <span className="ql-view-badge">{quote.view_count}×</span>}
 </div>
 <span className="qlp-fs-xs-b2de">{formatDate(quote.updated_at || quote.created_at)}</span>
 </div>
 </Link>
 );
}
...
function TableHeader() {
 return (
 <div className="qlp-grid_fs-2xs-882c">
 <span>Quote #</span>
 <span>Title</span>
 <span>Status</span>
 <span className="qlp-ta-right-a1ae">Total</span>
 <span className="qlp-ta-right-a1ae">Updated</span>
 </div>
 );
}
...
export default function QuotesListPage() {
 ...
 <input
 type="search"
 input qlp-s3-d14fclassName="input"
 placeholder="Search quotes…"
 value={search}
 ql-desktop-filters qlp-flex-132de => setSearch(e.target.value)}
 
 aria-label="Search quotes"
 />
 <div className="ql-desktop-filters" 'center' }}>
 <select
 className="input"
 value={sortBy}
 ...
 <label className="qlp-flex_fs-xs-dd0b">
 <input type="checkbox" checked={hideCompleted} onChange={toggleHideCompleted} className="qlp-s1-5124" />
 Hide completed
 </label>
 </div>
 </div>
 ...
 onClick={() => { setSearch(''); setStatusFilter(null); setHideCompleted(false); try { localStorage.removeItem('pl_hide_completed');panel qlp-s0-d341tch (e) { console.warn("[PL]", e); } }}
 >
 Clear filters
 </button>
 </div>
