    ...
    >
      <div className="ql-row-info">
        <span className="ql-row-title">
          ...
          {quote.customer?.name && (
            <span style={{ fontWeight: 400, color: 'var(--muted)', marginLeft: 8, fontSize: 'var(--text-sm)' }}>
              · {quote.customer.name}
            </span>
          )}
        </span>
        <span className="ql-row-meta">
          {num}
          {monthly && <> · <strong style={{ color: 'var(--brand)', fontWeight: 700 }}>{currency(monthly)}/mo</strong></>}
          <span style={{ marginLeft: 6, opacity: 0.6 }}>{formatDate(quote.updated_at || quote.created_at)}</span>
        </span>
      </div>
      <span className={`chip chip-${status}`}>
        {chipForStatus(status)}
        {viewBadge && <span style={{ marginLeft: 4, opacity: 0.7 }}>{quote.view_count}×</span>}
      </span>
  ...
  return (
    ...
    >
      <div className="ql-row-info">
        ...
        <span className="ql-row-meta">
          {hasCustomer ? quote.customer.name : <span style={{ color: 'var(--muted)' }}>Draft</span>}
      ...
      <div className="ql-row-right">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {viewBadge && <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)', fontWeight: 600 }}>{quote.view_count}×</span>}
        </div>
      </div>
    </Link>
  );
