          ...
          <Card padding="loose" className="qb-zone pl-describe-stable" elevation={1}>
            ...
            <details style={{ marginTop: 4 }}>
              ...
              <div className="jd-row">
                <div className="jd-section"><select className="jd-input jd-select" value={trade} onChange={e => setTrade(e.target.value)} aria-label="Trade">{TRADES.map(t => <option key={t}>{t}</option>)}</select></div>
                <div className="jd-section"><select className="jd-input jd-select" value={province} onChange={e => setProvince(e.target.value)} aria-label="Province">{(country === 'US' ? US_STATES : CA_PROVINCES).map(p => <option key={p}>{p}</option>)}</select></div>
              </div>
            </details>
            ...
            <div className="jd-footer">
              ...
              }} disabled={!description.trim()} className="btn-link">or add items manually <ChevronRight size={14} /></button>
            </div>
          </Card>
        )}
        ...
        {phase === 'building' && (() => {
          ...
          <Card padding="loose" className="pl-building-stable" elevation={1}>
            ...
            <div className="bs-loading">
              ...
              {photoSaved && <div className="jd-photo-saved">✓ Photo included</div>}
              <div className="bs-skeleton-list">
                {previewItems.map((name, i) => (
                  <div key={`sk-${name}-${i}`} className="bs-skeleton-item">
          ...
          <div style={isLocked ? { pointerEvents: 'none', opacity: 0.65 } : undefined}>
              ...
              <div className="rq-builder-right">
                ...
                {lineItems.length > 0 && grandTotal > 0 && (
                  <div className="rq-close-tips">
                    ...
                    {!draft.deposit_required && <div className="rq-close-tip">○ Add a deposit to lock in the job</div>}
                  </div>
                )}
                {/* Tracking teaser */}
                {lineItems.length > 0 && grandTotal > 0 && (
            ...
            <div className="rq-footer">
              ...
              <div id="qb-send-btn" className="rq-footer-right">
                ...
                {itemCount === 0 ? (
                  <button className="btn btn-primary btn-lg">Add items to send</button>
                ) : !draft.customer_id ? (
                ...
                <div className="qb-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
                  ...
                  <div className="rq-send-body">
                    ...
                    <div style={{ marginTop: 12 }}>
                      <label className="jd-label">Send via</label>
