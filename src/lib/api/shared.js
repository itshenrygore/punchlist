...
export function friendly(err) {
  ...
  if (msg.includes('coerce') || msg.includes('single')) return 'Record not found. It may have been deleted.';
  if (msg.includes('duplicate key')) return 'This record already exists.';
  if (msg.includes('violates foreign key')) return 'A linked record is missing. Try refreshing.';
  ...
  if (msg.includes('rate limit') || msg.includes('429')) return 'Too many requests. Wait a moment and try again.';
  if (msg.includes('column') && msg.includes('does not exist')) return 'Database update needed. Contact hello@punchlist.ca.';
  if (/[_]|SELECT|INSERT|UPDATE|DELETE|FROM|WHERE/i.test(msg)) return 'Something went wrong. Please try again.';
