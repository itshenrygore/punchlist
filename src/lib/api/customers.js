...
export async function findCustomerMatches(userId, text) {
  if (!text || text.length < 2) return [];
  const q = supabase.from('customers').select('id,name,email,phone,address').ilike('name', `%${text}%`).limit(10);
  if (userId) q.eq('user_id', userId);
  const { data, error } = await q;
  if (error) return [];
  return data || [];
...
export async function mergeCustomers(survivorId, victimId, userId) {
  if (userId) {
    const { data: s } = await supabase.from('customers').select('id').eq('id', survivorId).eq('user_id', userId).maybeSingle();
    const { data: v } = await supabase.from('customers').select('id').eq('id', victimId).eq('user_id', userId).maybeSingle();
    if (!s || !v) throw new Error('Customer not found');
  }
