...
export async function getWonQuoteContext(_userId, limit = 5) {
  try {
    const { data } = await supabase
      ...
      .in('status', ['approved', 'approved_pending_deposit', 'deposit_paid', 'converted_to_invoice', 'paid'])
      .order('updated_at', { ascending: false })
      .limit(limit);
...
export async function getFrequentLineItems(userId, limit = 10) {
  if (!userId) return [];
  try {
    const { data } = await supabase
      .from('quotes')
      .select('line_items(name, unit_price, category)')
      .eq('user_id', userId)
      .neq('status', 'draft')
      .order('created_at', { ascending: false })
      .limit(20);
    ...
    for (const q of data) {
      for (const item of (q.line_items || [])) {
        ...
        if (!key) continue;
        if (!freq[key]) freq[key] = { name: item.name, unit_price: item.unit_price, category: item.category, count: 0 };
        freq[key].count++;
      }
    }
...
export async function createInvoiceFromQuote(userId, quote, opts = {}) {
  ...
  const lineItems = (quote.line_items || []).filter(i => i.included !== false);
  if (lineItems.length) {
