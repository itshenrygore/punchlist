...
export const FAQ = [
  {
    ...
    a: 'No. The free plan has no credit card, no time limit, and no strings. Build up to 5 quotes per month.',
  },
  {
    q: 'What happens when I hit the free limit?',
    a: 'Your existing quotes stay active — customers can still view and approve them. To send new quotes, upgrade to Pro or wait for next month.',
  },
  {
    q: 'Can I switch from monthly to annual?',
    a: 'Yes, anytime from your billing page. You get credit for unused time on your monthly plan.',
  },
  {
    q: 'Does this work for my trade?',
    a: 'Plumbing, electrical, HVAC, roofing, painting, carpentry, general contracting, handyman, and more. The catalog was built with tradespeople.',
  },
  {
    q: 'Can I cancel anytime?',
    a: 'Yes. Cancel from your billing page. You keep access through the end of your billing period.',
  },
];
...
export function countSentThisMonth(quotes) {
  ...
  return (quotes || []).filter(q => {
    ...
    // If sent_at exists and is this month, always count it (even if created_at is older)
    if (q.sent_at) return tsTime >= monthStart;
    // Fallback: use created_at only if no sent_at recorded
    return tsTime >= monthStart;
  }).length;
}
