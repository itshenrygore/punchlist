    ...
    <div style={overlayStyle} onClick={onClose}>
      ...
      >
        ...
        <div style={headerStyle}>
          <div>
            <div style={{ fontSize: '1.05rem', fontWeight: 700, letterSpacing: '-.02em', color: 'var(--text)' }}>{title}</div>
            {displayLabel && (
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--muted)', marginTop: 2 }}>{displayLabel}</div>
            )}
          </div>
        ...
        <form style={bodyStyle} onSubmit={handleSave}>
          ...
          <div>
            ...
            {scheduledFor && (
              <div style={{ fontSize: 'var(--text-base)', fontWeight: 700, color: 'var(--text)', marginBottom: 6, letterSpacing: '-.01em' }}>
                {(() => {
                  try {
                ...
                <div style={dropdownStyle}>
                  {filteredCustomers.map(c => (
                    <div key={c.id} style={dropdownItemStyle} onClick={() => selectCustomer(c)}>
                      <span>{c.name}</span>
                      {c.phone && <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>{c.phone}</span>}
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {customerQuotes.map(q => (
                  ...
                  >
                    <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600 }}>{q.title || 'Untitled'}</span>
                    <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>
          ...
          {/* Crew assignment */}
          <div>
            <span style={labelStyle}>Assigned to <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: 'var(--muted)' }}>(optional)</span></span>
            <input
              style={inputStyle}
              value={assignedTo}
            ...
            <div style={conflictStyle}>
              ...
              {suggestedSlots.length > 0 && (
                <div style={{ marginTop: 8 }}>
                  <span style={{ fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>Available:</span>
              ...
              <div style={{ marginTop: 6, fontSize: 'var(--text-2xs)', color: 'var(--muted)' }}>
